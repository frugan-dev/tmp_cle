...
Sql::initQuery(DB_TABLE_PREFIX.'team_config', ['*'], [], '');
...
if (Core::$resultOp->error == 0) {
    switch (Core::$request->method) {
        ...
        case 'dt':
            ...
            Sql::initQuery(DB_TABLE_PREFIX.'team', ['*'], [$id], 'active = 1 AND id = ?', '');
            ...
            if ($App->item->meta_description == '' && $App->item->content != '') {
                $App->item->meta_description = ToolsStrings::getStringFromTotNumberChar($App->item->content, ['numchars' => 150]);
            }
            ...
            if ($App->moduleData->imageheader != '') {
                $App->meta_og_image = UPLOAD_DIR.'news/'.$App->moduleData->imageheader;
            }
            if ($App->item->filename != '') {
                $App->meta_og_image = UPLOAD_DIR.'news/'.$App->item->filename;
            }
        ...
        default:
            ...
            while ($row = $pdoObject->fetch()) {
                $row->role = Multilanguage::getLocaleObjectValue($row, 'role_', Config::$langVars['user'], ['htmLawed' => 1,'parse' => 1]);
                $row->summary = Multilanguage::getLocaleObjectValue($row, 'summary_', Config::$langVars['user'], ['htmLawed' => 1,'parse' => 1]);
                $row->universita = Multilanguage::getLocaleObjectValue($row, 'universita_', Config::$langVars['user'], ['htmLawed' => 1,'parse' => 1]);
                $App->items[] = $row;
            }
