...
if (Core::$resultOp->error == 0) {
    switch (Core::$request->method) {
        ...
        case 'vendor':
            ...
            $id = intval($App->param);
            if ($id > 0) {
                if (isset($App->params[0])) {
                    $App->targaStato = $App->params[0];
                }
                /* preleva i dati della voce */
                Sql::initQuery(DB_TABLE_PREFIX.'vendors', ['*'], [$id], 'active = 1 AND id = ?');
                $App->item = Sql::getRecord();
                ...
                if (Core::$resultOp->error == 0) {
                    if ($App->listStates == true) {
                        ...
                        if (Core::$resultOp->error == 0) {
                            ...
                            if ($where2 != '') {
                                $where1 = $where1.' AND ('.rtrim($where2, ' OR ').')';
                            }
                        ...
                        if (is_array($App->vendorsList) && count($App->vendorsList) > 0) {
                            foreach ($App->vendorsList as $value) {
                                ...
                                if ($value->latitude != '' && $value->longitude != '') {
                                    $centriJsc .= "['".$value->name."', ".$value->latitude.','.$value->longitude.",3,'".$note."'],";
                                }
                            }
...
switch ($App->view) {
    ...
    case 'error':
        ...
        if (Core::$resultOp->message == '') {
            Core::$resultOp->message = $_lang['sono presenti uno o più errori!'];
        }
