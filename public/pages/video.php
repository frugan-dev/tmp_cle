...
if (isset($dataMenuPages[$App->modulePageData->alias]->breadcrumbs) && is_array($dataMenuPages[$App->modulePageData->alias]->breadcrumbs) && count($dataMenuPages[$App->modulePageData->alias]->breadcrumbs) > 0) {
    array_pop($dataMenuPages[$App->modulePageData->alias]->breadcrumbs);
}
/* aggiorna breadcrumbs */
if (isset($dataMenuPages[$App->modulePageData->alias]->breadcrumbs)) {
    $breadcrumbs = $dataMenuPages[$App->modulePageData->alias]->breadcrumbs;
}
$x = 1;
...
if (is_object($App->modulePageData)) {
    if (isset($App->modulePageData->filename) && $App->modulePageData->filename != '') {
        $App->modulePageData->image_top =  UPLOAD_DIR.'pages/'.$App->modulePageData->filename;
    }
    if (isset($App->modulePageData->filename1) && $App->modulePageData->filename1 != '') {
        $App->modulePageData->image_bottom =  UPLOAD_DIR.'pages/'.$App->modulePageData->filename1;
    }
}
            ...
            if (Core::$resultOp->error == 0) {
                ...
                if (is_array($obj) && is_array($obj) && count($obj) > 0) {
                    foreach ($obj as $value) {
                        ...
                        if (isset($match[1][0])) {
                            $value->embedded_url = $match[1][0];
                        }

                        $value->url_image_preview = '';
                        if ($value->filename != '') {
                            $value->url_image_preview = UPLOAD_DIR.'video/'.$value->filename;
                        }
                        if ($value->url_video_image_preview != '') {
                            $value->url_image_preview = $value->url_video_image_preview;
                        }
