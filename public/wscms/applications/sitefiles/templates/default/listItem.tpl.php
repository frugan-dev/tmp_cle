<!-- site-files/listItem.tpl.php v.1.0.0. 22/06/2016 -->
<div class="row">
	<div class="col-md-3 new">
		<a href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/newItem" title="{{ Lang['inserisci un nuovo %ITEM%']|replace({'%ITEM%':Lang['file'] })|capitalize }}" class="btn btn-sm btn-primary">{{ Lang['nuovo %ITEM%']|replace({'%ITEM%':Lang['file'] })|capitalize }}</a>
	</div>
							...
							{% if App.items is iterable and App.items|length > 0 %}
								{% for key,value in App.items %}
									<tr>
										{% if (App.userLoggedData.is_root is defined) and (App.userLoggedData.is_root is same as(1)) %}	
											<td>{{ value.id }}</td>
											<td>{{ value.id-folder }}</td>
										{% endif %}
										
										<td>{{ value.title_it }}</td>
										<td>	
											<a class="" href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/downloadItem/{{ value.id }}" title="Scarica il file">
											{{ value.org_filename }}
											</a>
