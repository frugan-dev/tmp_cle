...
/*
    ...
    site-files/items.php v.2.6.3. 18/04/2016
*/
...
if (isset($_POST['id_folder'])) {
    $_MY_SESSION_VARS = $my_session->addSessionsModuleSingleVar($_MY_SESSION_VARS, $App->sessionName, 'id_folder', $_POST['id_folder']);
}

if (Core::$request->method == 'listItem' && $App->id > 0) {
    $_MY_SESSION_VARS = $my_session->addSessionsModuleSingleVar($_MY_SESSION_VARS, $App->sessionName, 'id_folder', $App->id);
}

/* gestione sessione -> id_cat */
$App->id_folder = ($_MY_SESSION_VARS[$App->sessionName]['id_folder'] ?? 0);

//echo 'id folder: '.$App->id_folder;

Sql::initQuery($App->tableFold, ['id','title_it'], []);
...
if (!is_array($App->foldersData) || (is_array($App->foldersData) && count($App->foldersData) == 0)) {
    ToolsStrings::redirect(URL_SITE_ADMIN.Core::$request->action.'/messageFold/2/'.urlencode('Devi creare o attivare almeno un'.$App->labels['item']['ownerSex'].' '.$App->labels['item']['owner'].'!'));
    die();
}
...
switch (Core::$request->method) {
    case 'activeItem':
    case 'disactiveItem':
        Sql::manageFieldActive(substr((string) Core::$request->method, 0, -4), $App->params->tableItem, $App->id, ['label' => $_lang['file'],'attivata' => $_lang['attivato'],'disattivata' => $_lang['disattivato']]);
    ...
    case 'deleteItem':
        if ($App->id > 0) {
            ...
            Sql::initQuery($App->tableItem, ['id_folder','filename','folder_name'], [$App->id], 'id = ?');
    ...
    case 'insertItem':
        if ($_POST) {
            ...
            Sql::initQuery($App->tableFold, ['*'], [$_POST['id_folder']], 'id = ?');
    ...
    case 'updateItem':
        if ($_POST) {
            ...
            Sql::initQuery($App->tableItem, ['filename','org_filename','folder_name'], [$App->id], 'id = ?');
            $App->itemOld = Sql::getRecord();
            if (Core::$resultOp->error == 0) {
                ...
                $_POST['folder_name'] = '';
                if (isset($_POST['id_folder'])) {
                    Sql::initQuery($App->tableFold, ['folder_name'], [$_POST['id_folder']], 'id = ?');
                    ...
                    if (Core::$resultOp->error == 0) {
                        ...
                        if (Core::$resultOp->error == 0) {
                            if ($uploadFilename != '') {
                                ...
                                if (file_exists($App->itemUploadPathDir.$oldFolder_name.$App->itemOld->filename)) {
                                    @unlink($App->itemUploadPathDir.$oldFolder_name.$App->itemOld->filename);
                                }
                            }
                            ...
                            if ($uploadFilename == '' && $_POST['folder_name'] != $oldFolder_name && $App->itemOld->filename != '') {
                                ...
                                if (file_exists($App->itemUploadPathDir.$oldFolder_name.$App->itemOld->filename)) {
                                    copy($App->itemUploadPathDir.$oldFolder_name.$App->itemOld->filename, $App->itemUploadPathDir.$_POST['folder_name'].$App->itemOld->filename) or die('Errore spostamento file');
                                }
                                /* cancella l'immagine vecchia */
                                if (file_exists($App->itemUploadPathDir.$oldFolder_name.$App->itemOld->filename)) {
                                    @unlink($App->itemUploadPathDir.$oldFolder_name.$App->itemOld->filename);
    ...
    case 'downloadItem':
        if ($App->id > 0) {
            $renderTpl = false;
            ToolsUpload::downloadFileFromDB($App->itemUploadPathDir, $App->params->tableItem, $App->id, 'filename', 'org_filename', 'folder_name', '');
            die();
        }
        $App->viewMethod = 'list';
        break;
...
switch ((string)$App->viewMethod) {
    ...
    case 'list':
        ...
        $App->pageSubTitle = 'lista dei '.$App->labels['item']['items'].' presenti nell'.$App->labels['item']['ownerSex'].' '.$App->labels['item']['owner'];
