...
switch (Core::$request->method) {
    case 'moreOrderingIfil':
        $Utilities::increaseFieldOrdering($App->id, $_lang, ['table' => $App->params->tables['resources'],'orderingType' => $App->params->ordersType['ifil'],'parent' => 1,'parentField' => 'id_owner','label' => ucfirst((string) $_lang['file']).' '.$_lang['spostato'],'addclauseparent' => 'resource_type = ?','addclauseparentvalues' => [2]]);
        $App->viewMethod = 'list';
        break;
    case 'lessOrderingIfil':
        $Utilities::decreaseFieldOrdering($App->id, $_lang, ['table' => $App->params->tables['resources'],'orderingType' => $App->params->ordersType['ifil'],'parent' => 1,'parentField' => 'id_owner','label' => ucfirst((string) $_lang['file']).' '.$_lang['spostato'],'addclauseparent' => 'resource_type = ?','addclauseparentvalues' => [2]]);
        $App->viewMethod = 'list';
        break;
    ...
    case 'downloadIfil':
        if ($App->id > 0) {
            $renderTpl = false;
            ToolsDownload::downloadFileFromDB($App->params->uploadPaths['ifil'], $App->id);
            die();
        }
        $App->viewMethod = 'list';
        break;
