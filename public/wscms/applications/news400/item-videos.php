...
/* wscms/news/item-videos.php v.3.5.2. 20/08/2017 */
...
if (Core::$request->method == 'listIvid' && $App->id > 0) {
...
switch (Core::$request->method) {
    ...
    case 'downloadIvid':
        if ($App->id > 0) {
            $renderTpl = false;
            ToolsUpload::downloadFileFromDB($App->params->uploadPaths['pimg'], $App->params->tables['resources'], $App->id, 'filename', 'org_filename', '', '');
            die();
        }
        $App->viewMethod = 'list';
        break;
...
switch ((string)$App->viewMethod) {
    ...
    case 'formNew':
        ...
        $App->templateApp = 'formIvid.tpl.php';
        $App->methodForm = 'insertIvid';
        $App->jscript[] = '<script src="'.URL_SITE_ADMIN.$App->pathApplication.Core::$request->action.'/templates/'.$App->templateUser.'/js/formIvid.js"></script>';
        break;
    ...
    case 'list':
        ...
        $App->jscript[] = '<script src="'.URL_SITE_ADMIN.$App->pathApplication.Core::$request->action.'/templates/'.$App->templateUser.'/js/listIvid.js"></script>';
        break;
