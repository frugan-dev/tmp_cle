...
/* wscms/news/item-gallery.php v.3.5.2. 20/08/2017 */
...
if (Core::$request->method == 'listIgal' && $App->id > 0) {
...
switch (Core::$request->method) {
    ...
    case 'newIgal':
        $App->pageSubTitle .= $_lang['inserisci immagine galleria'];
    ...
    case 'insertIgal':
        if ($_POST) {
            ...
            if (!isset($_POST['resource_type'])) {
                $_POST['resource_type'] = 3;
            }
            if (!isset($_POST['code'])) {
                $_POST['code'] = '';
            }
            if (!isset($_POST['ordering']) || (isset($_POST['ordering']) && $_POST['ordering'] == 0)) {
                $_POST['ordering'] = Sql::getMaxValueOfField($App->params->tables['resources'], 'ordering', 'id_owner = '.intval($_POST['id_owner']).' AND resource_type = 3') + 1;
            }
            ...
            if (Core::$resultOp->error == 0) {
                ...
                if (Core::$resultOp->error == 0) {
                    ...
                    if (Core::$resultOp->error == 0) {
                        /* sposto il Igal */
                        if ($_POST['filename'] != '') {
                            move_uploaded_file(ToolsUpload::getTempFilename(), $App->params->uploadPaths['igal'].$_POST['filename']) or die('Errore caricamento file');
                        }
                    }
                }
            }
        ...
        if (Core::$resultOp->error > 0) {
            ...
            $App->viewMethod = 'list';
            Core::$resultOp->message = ucfirst((string) $_lang['immagine galleria inserita']).'!';
        }
        break;

    case 'modifyIgal':
        $App->pageSubTitle .=  $_lang['modifica immagine galleria'];
    ...
    case 'updateIgal':
            ...
            if (Core::$resultOp->error == 0) {
                ...
                if (Core::$resultOp->error == 0) {
                    ...
                    if (Core::$resultOp->error == 0) {
                        ...
                        if (Core::$resultOp->error == 0) {
                            if ($uploadFilename != '') {
                                move_uploaded_file(ToolsUpload::getTempFilename(), $App->params->uploadPaths['igal'].$uploadFilename) or die('Errore caricamento file');
        ...
        if (Core::$resultOp->error == 1) {
            ...
            if (isset($_POST['submitForm'])) {
                $App->viewMethod = 'list';
                Core::$resultOp->message = ucfirst((string) $_lang['immagine galleria modificata']).'!';
            } else {
                if (isset($_POST['id'])) {
                    $App->id = $_POST['id'];
                    $App->pageSubTitle .= $_lang['modifica immagine galleria'];
    ...
    case 'downloadIgal':
        if ($App->id > 0) {
            $renderTpl = false;
            ToolsUpload::downloadFileFromDB($App->params->uploadPaths['igal'], $App->params->tables['resources'], $App->id, 'filename', 'org_filename', '', '');
            die();
        }
        $App->viewMethod = 'list';
        break;
...
switch ((string)$App->viewMethod) {
    ...
    case 'formNew':
        ...
        $App->templateApp = 'formIgal.tpl.php';
        $App->methodForm = 'insertIgal';
        $App->jscript[] = '<script src="'.URL_SITE_ADMIN.$App->pathApplication.Core::$request->action.'/templates/'.$App->templateUser.'/js/formIgal.js"></script>';
        break;
    ...
    case 'list':
        ...
        $App->jscript[] = '<script src="'.URL_SITE_ADMIN.$App->pathApplication.Core::$request->action.'/templates/'.$App->templateUser.'/js/listIgal.js"></script>';
        break;
