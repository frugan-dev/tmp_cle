...
switch (Core::$request->method) {
    ...
    case 'activeCate':
    case 'disactiveCate':
        Sql::manageFieldActive(substr((string) Core::$request->method, 0, -4), $App->params->tableCate, $App->id, ['label' => $_lang['pagina'],'attivata' => $_lang['attivata'],'disattivata' => $_lang['disattivata']]);
    ...
    case 'moreOrderingCate':
        Utilities::increaseFieldOrdering($App->id, $_lang, ['table' => $App->params->tableCate,'orderingType' => $App->params->orderingType,'parent' => 0,'parentField' => '','label' => ucfirst((string) $_lang['pagina']).' '.$_lang['spostata']]);
        $_SESSION['message'] = '0|'.Core::$resultOp->message;
        ToolsStrings::redirect(URL_SITE_ADMIN.Core::$request->action.'/listCate');
        break;
    case 'lessOrderingCate':
        Utilities::decreaseFieldOrdering($App->id, $_lang, ['table' => $App->params->tableCate,'orderingType' => $App->params->orderingType,'parent' => 0,'parentField' => '','label' => ucfirst((string) $_lang['pagina']).' '.$_lang['spostata']]);
        $_SESSION['message'] = '0|'.Core::$resultOp->message;
        ToolsStrings::redirect(URL_SITE_ADMIN.Core::$request->action.'/listCate');
        break;
