...
<div class="row">
	<div class="col-md-3 new">
		<a href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/newCate" title="{{ Lang['inserisci una nuova %ITEM%']|replace({'%ITEM%':Lang['pagina'] })|capitalize }}" class="btn btn-sm btn-primary">{{ Lang['nuova %ITEM%']|replace({'%ITEM%':Lang['pagina'] })|capitalize }}</a>
	</div>
								...
								{% for key,value in App.items %}
									<tr>										
										...
										<a class="" href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/{{ App.params.orderingType == 'DESC' ? 'more' : 'less' }}OrderingCate/{{ value.id }}" title="{{ Lang['sposta']|capitalize }} {{ App.params.orderingType == 'DESC' ? Lang['giu'] : Lang['su'] }}"><i class="fas fa-long-arrow-alt-up"></i></a>								
  										...
  										<td><a href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/listItem/{{ value.id }}" title="Vai ai contenuti della pagina"><i class="fas fa-th-list"></i> - {{ value.numitems }}</a></td>
