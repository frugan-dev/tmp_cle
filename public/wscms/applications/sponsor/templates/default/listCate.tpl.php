<!-- news/listCate.tpl.php v.2.6.3. 06/04/2016 -->
<div class="row">
	<div class="col-md-3 new">
		<a href="<?php echo URL_SITE_ADMIN; ?><?php echo Core::$request->action; ?>/newCate" title="Inserisci un nuov<?php echo $this->App->labels['cate']['itemSex']; ?> <?php echo $this->App->labels['cate']['item']; ?>" class="btn btn-primary">Nuov<?php echo $this->App->labels['cate']['itemSex']; ?> <?php echo $this->App->labels['cate']['item']; ?></a>
	</div>
...
<form role="form" action="<?php echo URL_SITE_ADMIN; ?><?php echo Core::$request->action; ?>/listCate" method="post" enctype="multipart/form-data">	
	<div class="row">
		<div class="col-md-12">
							...
							<div class="tables_filter text-right">
								<label>
									...
									<input name="searchFromTable" value="<?php if (isset($this->mySessionVars[$this->App->sessionName]['srcTab']) && $this->mySessionVars[$this->App->sessionName]['srcTab'] != '') {
									    echo htmlspecialchars((string) $this->mySessionVars[$this->App->sessionName]['srcTab'], ENT_QUOTES, 'UTF-8');
					...
					<table class="table table-striped table-bordered table-hover listData">
						<thead>
							<tr>
								...
								<?php endif; ?>	
								<th>Titolo</th>
								<th><?php echo $this->App->labels['cate']['sons']; ?></th>						
								<th></th>
							</tr>
						</thead>
						<tbody>				
							<?php if (is_array($this->App->items) && count($this->App->items) > 0): ?>
								<?php foreach ($this->App->items as $key => $value): ?>
									<tr>										
										...
										<td class="actions">
											<a class="btn btn-default btn-circle" href="<?php echo URL_SITE_ADMIN; ?><?php echo Core::$request->action; ?>/<?php echo($value->active == 1 ? 'disactive' : 'active'); ?>Cate/<?php echo $value->id; ?>" title="<?php echo($value->active == 1 ? 'Disattiva' : 'Attiva'); ?>"><i class="fa fa-<?php echo($value->active == 1 ? 'unlock' : 'lock'); ?>"> </i> </a>			 
											<a class="btn btn-default btn-circle" href="<?php echo URL_SITE_ADMIN; ?><?php echo Core::$request->action; ?>/modifyCate/<?php echo $value->id; ?>" title="Modifica"><i class="fa fa-edit"> </i> </a>
											<a onclick="bootbox.confirm();" class="btn btn-default btn-circle confirm" href="<?php echo URL_SITE_ADMIN; ?><?php echo Core::$request->action; ?>/deleteCate/<?php echo $value->id; ?>" title="Cancella"><i class="fa fa-cut"> </i></a>
										</td>							
									</tr>	
								<?php endforeach; ?>
							<?php else: ?>
							...
							<div class="dataTables_paginate paging_simple_numbers" id="dataTables_paginate">
								<ul class="pagination">
									<li class="paginate_button previous<?php if ($this->App->pagination->page == 1) {
										...
										<a href="<?php echo URL_SITE_ADMIN; ?><?php echo Core::$request->action; ?>/pageCate/<?php echo $this->App->pagination->itemPrevious; ?>">Precedente</a>
									</li>
									
									<?php if (is_array($this->App->pagination->pagePrevious)): ?>
										<?php foreach ($this->App->pagination->pagePrevious as $key => $value): ?>
											<li><a href="<?php echo URL_SITE_ADMIN; ?><?php echo Core::$request->action; ?>/pageCate/<?php echo $value; ?>"><?php echo $value; ?></a></li>
										<?php endforeach; ?>
									<?php endif; ?>
										
									<li class="active"><a href="<?php echo URL_SITE_ADMIN; ?><?php echo Core::$request->action; ?>/pageCate/<?php echo $this->App->pagination->page; ?>"><?php echo $this->App->pagination->page; ?></a></li>
									...
									<li class="paginate_button next <?php if ($this->App->pagination->page >= $this->App->pagination->totalpage) {
										...
										<a href="<?php echo URL_SITE_ADMIN; ?><?php echo Core::$request->action; ?>/pageCate/<?php echo $this->App->pagination->itemNext; ?>">Prossima</a>
									</li>
								</ul>
							</div>
						</div>
