...
/* wscms/sponsor/items.php v.1.0.0. 28/06/2016
*/
...
if (isset($_POST['id_cat'])) {
    $_MY_SESSION_VARS = $my_session->addSessionsModuleSingleVar($_MY_SESSION_VARS, $App->sessionName, 'id_cat', $_POST['id_cat']);
}

if (Core::$request->method == 'listItem' && $App->id > 0) {
...
switch (Core::$request->method) {
    ...
    case 'moreOrderingItem':
        Utilities::increaseFieldOrdering($App->id, $_lang, ['table' => $App->tableItem,'orderingType' => $App->params->orderingType,'parent' => 0,'parentField' => '','label' => ucfirst((string) $_lang['voce']).' '.$_lang['spostato']]);
        $_SESSION['message'] = '0|'.Core::$resultOp->message;
        ToolsStrings::redirect(URL_SITE_ADMIN.Core::$request->action.'/listItem');
        break;
    case 'lessOrderingItem':
        Utilities::decreaseFieldOrdering($App->id, $_lang, ['table' => $App->tableItem,'orderingType' => $App->params->orderingType,'parent' => 0,'parentField' => '','label' => ucfirst((string) $_lang['voce']).' '.$_lang['spostato']]);
        $_SESSION['message'] = '0|'.Core::$resultOp->message;
        ToolsStrings::redirect(URL_SITE_ADMIN.Core::$request->action.'/listItem');
        break;
    ...
    case 'activeItem':
    case 'disactiveItem':
        Sql::manageFieldActive(substr((string) Core::$request->method, 0, -4), $App->tableItem, $App->id, ['label' => $_lang['voce'],'attivata' => $_lang['attivato'],'disattivata' => $_lang['disattivato']]);
        $_SESSION['message'] = '0|'.Core::$resultOp->message;
        ToolsStrings::redirect(URL_SITE_ADMIN.Core::$request->action.'/listItem');
        break;
        ...
        if ($App->id > 0) {
            ...
            if ($delete == true && Core::$resultOp->error == 0) {
                ...
                if (Core::$resultOp->error == 0) {
                    ...
                    Sql::deleteRecord();
                    if (Core::$resultOp->error == 0) {
                        if (isset($App->itemOld->filename) && file_exists($App->params->itemUploadPathDir.$App->itemOld->filename)) {
                            @unlink($App->params->itemUploadPathDir.$App->itemOld->filename);
                        }
    ...
    case 'newItem':
        ...
        $App->item->ordering = Sql::getMaxValueOfField($App->tableItem, 'ordering', '') + 1;
    ...
    case 'insertItem':
        ...
        if ($_POST) {
            ...
            if (!isset($_POST['ordering']) || (isset($_POST['ordering']) && $_POST['ordering'] == 0)) {
                $_POST['ordering'] = Sql::getMaxValueOfField($App->tableItem, 'ordering', 'id_cat = '.intval($_POST['id_cat'])) + 1;
            }
            /* preleva il filename dal form */
            ToolsUpload::setFilenameFormat(['jpg','png','JPG']);
            ...
            if (Core::$resultOp->error == 0) {
                ...
                if (Core::$resultOp->error == 0) {
                    ...
                    if (Core::$resultOp->error == 0) {
                        /* sposto il file */
                        if ($_POST['filename'] != '') {
                            move_uploaded_file(ToolsUpload::getTempFilename(), $App->params->itemUploadPathDir.$_POST['filename']) or die('Errore caricamento file');
                        }
                    }
                }
            }
            ...
            if (Core::$resultOp->error == 0) {
                ...
                if (Core::$resultOp->error == 0) {
                    ...
                    if (isset($_POST['deleteFilename']) && $_POST['deleteFilename'] == 1) {
                        if (file_exists($App->params->itemUploadPathDir.$App->itemOld->filename)) {
                            @unlink($App->params->itemUploadPathDir.$App->itemOld->filename);
                        }
                        $_POST['filename'] = '';
                        $_POST['org_filename'] = '';
                    ...
                    if (Core::$resultOp->error == 0) {
                        ...
                        if (Core::$resultOp->error == 0) {
                            if ($uploadFilename != '') {
                                move_uploaded_file(ToolsUpload::getTempFilename(), $App->params->itemUploadPathDir.$uploadFilename) or die('Errore slavataggio file '.$App->params->itemUploadPathDir.$uploadFilename);
...
switch ((string)$App->viewMethod) {
    case 'formNew':
        ...
        if (Core::$resultOp->error > 0) {
            Utilities::setItemDataObjWithPost($App->item, $App->fieldsItem);
        }
    ...
    case 'list':
        ...
        $qryFields[] = 'ite.*';
        if ($App->params->item_files == 1) {
            $qryFields[] = '(SELECT COUNT(fil.id) FROM '.$App->tableIfil.' AS fil WHERE fil.id_owner = ite.id) AS files';
        }
