<!-- site-images/listImages.tpl.php v.2.6.3. 05/04/2016 -->
<div class="row">
	<div class="col-md-3 new">
		<a href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/newItem" title="{{ Lang['inserisci una nuova %ITEM%']|replace({'%ITEM%':Lang['immagine'] })|capitalize }}" class="btn btn-sm btn-primary">{{ Lang['nuova %ITEM%']|replace({'%ITEM%':Lang['immagine'] })|capitalize }}</a>
	</div>
		...
		<form role="form" action="{{ URLSITEADMIN }}{{ CoreRequest.action }}/listItem" method="post" enctype="multipart/form-data">
			...
			<div class="form-group row">
				...
				<div class="col-sm-12 col-md-6 col-lg-3 col-xl-4">
					...
					<select class="form-control input-md" name="id_folder" onchange="this.form.submit();">
						<option 
						value="0"
						{% if App.id_folder == 0 %} selected="selected"{% endif %}
						>Tutte</option>

							{% if App.foldersData is iterable and App.foldersData|length > 0 %}	
								{% for key,value in App.foldersData %}											
									<option value="{{ value.id }}" {% if App.id_folder is defined and App.id_folder == value.id  %}selected="selected" {% endif %}>{{ value.title_it|e('html') }}</option>
								{% endfor %}
							{% endif %}
					</select>
				</div>
						...
						{% if App.items is iterable and App.items|length > 0 %}
							{% for key,value in App.items %}
								<tr>
									{% if (App.userLoggedData.is_root is defined) and (App.userLoggedData.is_root is same as(1)) %}	
										<td>{{ value.id }}</td>
										<td>{{ value.id_folder }}</td>
									{% endif %}
									
									<td>{{ value.title_it }}</td>
									<td>	
										<a class="" href="{{ App.itemUploadDir }}{{ value.folder_name }}{{ value.filename }}" data-lightbox="image-1" title="{{ value.org_filename }}">
											<img class="img-miniature"  src="{{ App.itemUploadDir }}{{ value.folder_name }}{{ value.filename }}" alt="{{ value.org_filename }}">
										</a>
									</td>	
									<td>												
										<button type="button" class="popoverInfo btn btn-default btn-circle" data-container="body" data-toggle="popover" data-placement="top" data-content="{{ App.itemUploadDir }}{{ value.folder_name }}{{ value.filename }}">
