 ...
 * admin/faq/items.php v.4.5.1. 26/11/2018
*/
...
switch (Core::$request->method) {
    ...
    case 'activeItem':
    case 'disactiveItem':
        Sql::manageFieldActive(substr((string) Core::$request->method, 0, -4), $App->params->tables['item'], $App->id, ['label' => $_lang['voce'],'attivata' => $_lang['attivata'],'disattivata' => $_lang['disattivata']]);
        $_SESSION['message'] = '0|'.Core::$resultOp->message;
        ToolsStrings::redirect(URL_SITE_ADMIN.Core::$request->action.'/listItem');
        break;
    ...
    case 'deleteItem':
        if ($App->id > 0) {
            ...
            $_SESSION['message'] = '0|'.ucfirst((string) preg_replace('/%ITEM%/', (string) $_lang['voce'], (string) $_lang['%ITEM% cancellata'])).'!';
            ToolsStrings::redirect(URL_SITE_ADMIN.Core::$request->action.'/listItem');
        } else {
    ...
    case 'insertItem':
        ...
        if ($_POST) {
            ...
            $_SESSION['message'] = '0|'.ucfirst((string) preg_replace('/%ITEM%/', (string) $_lang['voce'], (string) $_lang['%ITEM% inserita']));
            ToolsStrings::redirect(URL_SITE_ADMIN.Core::$request->action.'/listItem');
    ...
    case 'updateItem':
        if ($_POST) {
            ...
            if (Core::$resultOp->error == 0) {
                ...
                $_SESSION['message'] = '0|'.ucfirst((string) preg_replace('/%ITEM%/', (string) $_lang['voce'], (string) $_lang['%ITEM% modificata']));
