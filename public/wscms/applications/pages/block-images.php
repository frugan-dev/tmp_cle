...
if (Core::$request->method == 'listBimg' && $App->id > 0) {
...
if ($App->id_owner > 0) {
    Sql::initQuery($App->params->tables['iblo'], ['*'], [$App->id_owner], 'active = 1 AND id = ?');
...
if (!isset($App->ownerData->id) || (isset($App->ownerData->id) && $App->ownerData->id == 0)) {
    ToolsStrings::redirect(URL_SITE_ADMIN.Core::$request->action.'/messageIblo/2/'.urlencode((string) $_lang['Devi creare o attivare almeno una voce!']));
    die();
}
...
switch (Core::$request->method) {
    case 'moreOrderingBimg':
        Utilities::increaseFieldOrdering($App->id, $_lang, ['table' => $App->params->tables['resources'],'orderingType' => $App->params->ordersType['bimg'],'parent' => 1,'parentField' => 'id_owner','label' => ucfirst((string) $_lang['immagine']).' '.$_lang['spostata'],'addclauseparent' => 'resource_type = ?','addclauseparentvalues' => [1]]);
        $App->viewMethod = 'list';
        break;
    case 'lessOrderingBimg':
        Utilities::decreaseFieldOrdering($App->id, $_lang, ['table' => $App->params->tables['resources'],'orderingType' => $App->params->ordersType['bimg'],'parent' => 1,'parentField' => 'id_owner','label' => ucfirst((string) $_lang['immagine']).' '.$_lang['spostata'],'addclauseparent' => 'resource_type = ?','addclauseparentvalues' => [1]]);
        $App->viewMethod = 'list';
        break;
    ...
    case 'insertBimg':
        if ($_POST) {
            ...
            if (!isset($_POST['resource_type'])) {
                $_POST['resource_type'] = 1;
            }
            if (!isset($_POST['code'])) {
                $_POST['code'] = '';
            }
            if (!isset($_POST['ordering']) || (isset($_POST['ordering']) && $_POST['ordering'] == 0)) {
                $_POST['ordering'] = Sql::getMaxValueOfField($App->params->tables['resources'], 'ordering', 'id_owner = '.intval($_POST['id_owner']).' AND resource_type = 1') + 1;
            }
