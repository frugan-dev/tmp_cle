...
switch (Core::$request->method) {
    ...
    case 'ajaxMoveAddressCatToSendList':
        ...
        if (isset($_POST['listAddressCat'])) {
            $arr = explode(',', (string) $_POST['listAddressCat']);
                ...
                if (isset($obj) && is_array($obj) && count($obj) > 0) {
                    foreach ($obj as $value) {
                        ...
                        if (isset($value->email) && $value->email != '') {
                            ...
                            Sql::initQuery($App->tableIndInvio, ['*'], [$value->email], 'email = ?');
                            $foo = Sql::getRecords();
                            if (Core::$resultOp->error > 0) {
                                die('Errore database');
                                ToolsStrings::redirect(URL_SITE_ADMIN.'error/db');
                            ...
                            $count = count($foo);
                            if ($count == 0) {
                                Sql::initQuery($App->tableIndInvio, ['email','hash','inviata'], [$value->email,$value->hash,'0']);
                                Sql::insertRecord();
                                if (Core::$resultOp->error > 0) {
                                    die('Errore database 1');
                                    ToolsStrings::redirect(URL_SITE_ADMIN.'error/db');
                                }
                            }
                        }
    ...
    default:
        ...
        if (is_array($App->listAddressCat) && count($App->listAddressCat) > 0) {
            foreach ($App->listAddressCat as $key => $value) {
                ...
                Sql::setFieldsValue([$value->id]);
                $obj = Sql::getRecords();
                if (isset($obj[0]->indirizzi)) {
                    $value->numitems = $obj[0]->indirizzi;
                }
                $arr[] = $value;
            }
