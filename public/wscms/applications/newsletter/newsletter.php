...
switch (Core::$request->method) {
    ...
    case 'insertNew':
        if ($_POST) {
            ...
            if (!isset($_POST['datatimesend'])) {
                $_POST['datatimesend'] = Config::$nowDateTimeIso;
            }
        ...
        if (Core::$resultOp->error == 0) {
            ...
            if (file_exists($file) == true) {
                ...
                if (isset($App->item->datatimeins)) {
                    $App->item->finalOutput = preg_replace('/%DATATIMEINS%/', $App->item->datatimeins, (string) $App->item->finalOutput);
                }
                if (isset($App->item->title_it)) {
                    $App->item->finalOutput = preg_replace('/%TITLE%/', htmlspecialchars($App->item->title_it), (string) $App->item->finalOutput);
                }
                if (isset($App->item->content_it)) {
...
switch ((string)$App->viewMethod) {
    case 'formNew':
        ...
        if (Core::$resultOp->error > 0) {
            Utilities::setItemDataObjWithPost($App->item, $App->fieldsNew);
        }
