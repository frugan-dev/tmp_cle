			...
			<div class="tab-pane active" id="datibase">
				<fieldset>
					<div class="form-group">
						...
						<div class="col-sm-7">
							<input required type="text" name="name" class="form-control" id="nameID" placeholder="Inserisci un nome" value="<?php if (isset($this->App->item->name)) {
							    echo SanitizeStrings::cleanForFormInput($this->App->item->name);
							} ?>">
				    	</div>
					</div>
					<div class="form-group">
						...
						<div class="col-sm-7">
							<input required type="text" name="surname" class="form-control" id="surnameID" placeholder="Inserisci un cognome" value="<?php if (isset($this->App->item->surname)) {
							    echo SanitizeStrings::cleanForFormInput($this->App->item->surname);
							} ?>">
				    	</div>
					</div>
				</fieldset>
				...
				<fieldset>
					<div class="form-group">
						...
						<div class="col-sm-3">
							<input required type="email" name="email" class="form-control" id="emailID" placeholder="Inserisci un indirizzo email"  value="<?php if (isset($this->App->item->email)) {
							    echo SanitizeStrings::cleanForFormInput($this->App->item->email);
							} ?>">
				    	</div>
				...
				<?php if ($this->App->params->categories == 1) : ?>
				    ...
				    ?>
								...
								<select required name="id_cats[]" size="15" multiple id="id_catsID">
									...
									<option value="<?php echo $value->id; ?>"<?php if (is_array($itemCats) && in_array($value->id, $itemCats)) {
									    echo ' selected="selected"';
									} ?>><?php echo $value->id.' - '.htmlspecialchars((string) $value->title_it, ENT_QUOTES, 'UTF-8'); ?></option>
									<?php endforeach; ?>
								<?php endif; ?>
								</select>								
					    	</div>
...
<!-- sezione datibase -->	  
		...
		<div class="form-group">
			<div class="col-sm-offset-2 col-sm-7">
				<input type="hidden" name="active" id="activeID" value="<?php if (isset($this->App->item->active)) {
				    echo $this->App->item->active;
				} ?>">
				<input type="hidden" name="created" id="createdID" value="<?php if (isset($this->App->item->created)) {
				    echo $this->App->item->created;
				} ?>">
				<input type="hidden" name="id" id="idID" value="<?php if (isset($this->App->id)) {
				    echo $this->App->id;
				} ?>">
				<input type="hidden" name="hash" id="hashID" value="<?php if (isset($this->App->item->hash)) {
				    echo $this->App->item->hash;
				} ?>">
				<input type="hidden" name="dateconfirmed" id="dateconfirmedID" value="<?php if (isset($this->App->item->dateconfirmed)) {
				    echo $this->App->item->dateconfirmed;
				} ?>">
				<input type="hidden" name="confirmed" id="confirmedID" value="<?php if (isset($this->App->item->confirmed)) {
				    echo $this->App->item->confirmed;
				} ?>">					
