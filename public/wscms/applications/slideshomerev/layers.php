...
if (Core::$request->method == 'listLaye' && $App->id > 0) {
    $_MY_SESSION_VARS = $my_session->addSessionsModuleSingleVar($_MY_SESSION_VARS, $App->sessionName, 'slide_id', $App->id);
}
...
if (isset($_POST['id_cat']) && isset($_MY_SESSION_VARS[$App->sessionName]['id_cat']) && $_MY_SESSION_VARS[$App->sessionName]['id_cat'] != $_POST['id_cat']) {
    ...
    case 'insertLaye':
        if ($_POST) {
            ...
            if (!isset($_POST['ordering'])) {
                $_POST['ordering'] = Sql::getMaxValueOfField($App->params->tables['laye'], 'ordering', 'slide_id = '.intval($App->slide_id)) + 1;
            }
...
switch ((string)$App->viewMethod) {
    case 'formNew':
        ...
        if (Core::$resultOp->error > 0) {
            Utilities::setItemDataObjWithPost($App->item, $App->params->fields['laye']);
        }
