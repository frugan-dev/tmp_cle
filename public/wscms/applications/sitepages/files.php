...
switch (Core::$request->method) {
    case 'moreOrderingIfil':
        $Utilities::increaseFieldOrdering($App->id, ['table' => $App->tableIfil,'orderingType' => $App->orderingType,'parent' => true,'parentField' => 'id_owner','sexSuffix' => $App->labels['ifil']['itemSex'],'labelItem' => ucfirst((string) $App->labels['ifil']['item'])]);
        $App->viewMethod = 'list';
        break;
    case 'lessOrderingIfil':
        $Utilities::decreaseFieldOrdering($App->id, ['table' => $App->tableIfil,'orderingType' => $App->orderingType,'parent' => true,'parentField' => 'id_owner','sexSuffix' => $App->labels['ifil']['itemSex'],'labelItem' => ucfirst((string) $App->labels['ifil']['item'])]);
        $App->viewMethod = 'list';
        break;
    ...
    case 'insertIfil':
        if ($_POST) {
            ...
            $_POST['ordering'] = Sql::getMaxValueOfField($App->tableIfil, 'ordering', 'id_owner = '.$App->id_owner) + 1;
...
switch ((string)$App->viewMethod) {
    ...
    case 'formNew':
        ...
        if (Core::$resultOp->error > 0) {
            Utilities::setItemDataObjWithPost($App->item, $App->fieldsIfil);
        }
