...
class Module
{
    ...
    public function listMainData($fields, $page, $itemsForPage, $languages, $opz = [])
    {
        ...
        if ($opz['files'] == 1) {
            $qry .= ','.PHP_EOL.'(SELECT COUNT(fil.id) FROM '.$opz['tablefiles'].' AS fil WHERE fil.id_owner = c.id) AS files';
        }
        if ($opz['images'] == 1) {
            $qry .= ','.PHP_EOL.'(SELECT COUNT(img.id) FROM '.$opz['tableimages'].' AS img WHERE img.id_owner = c.id) AS images';
        }
