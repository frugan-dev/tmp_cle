...
switch (Core::$request->method) {
    ...
    case 'activeCate':
    case 'disactiveCate':
        Sql::manageFieldActive(substr((string) Core::$request->method, 0, -4), $App->params->tableCate, $App->id, ['label' => $_lang['categoria'],'attivata' => $_lang['attivata'],'disattivata' => $_lang['disattivata']]);
    ...
    case 'moreOrderingCate':
        Utilities::increaseFieldOrdering($App->id, $_lang, ['table' => $App->params->tableCate,'orderingType' => $App->params->orderingType,'parent' => 0,'parentField' => '','label' => ucfirst((string) $_lang['categoria']).' '.$_lang['spostata']]);
        $_SESSION['message'] = '0|'.Core::$resultOp->message;
        ToolsStrings::redirect(URL_SITE_ADMIN.Core::$request->action.'/listCate');
        break;
    case 'lessOrderingCate':
        Utilities::decreaseFieldOrdering($App->id, $_lang, ['table' => $App->params->tableCate,'orderingType' => $App->params->orderingType,'parent' => 0,'parentField' => '','label' => ucfirst((string) $_lang['categoria']).' '.$_lang['spostata']]);
        $_SESSION['message'] = '0|'.Core::$resultOp->message;
        ToolsStrings::redirect(URL_SITE_ADMIN.Core::$request->action.'/listCate');
        break;
    ...
    case 'deleteCate':
        if ($App->id > 0) {
            ...
            Sql::initQuery($App->params->tableItem, ['id'], [$App->id], 'id_cat = ?');
            ...
            if ($delete == true && Core::$resultOp->error == 0) {
                ...
                Sql::initQuery($App->params->tableCate, ['*'], [$App->id], 'id = ?');
                ...
                if (Core::$resultOp->error == 0) {
                    ...
                    if (Core::$resultOp->error == 0) {
                        ...
                        if (isset($App->itemOld->folder_name) && $App->itemOld->folder_name != '' && file_exists($App->itemUploadPathDir.$App->itemOld->folder_name)) {
    ...
    case 'insertCate':
        if ($_POST) {
            ...
            if (!isset($_POST['ordering']) || (isset($_POST['ordering']) && $_POST['ordering'] == 0)) {
                $_POST['ordering'] = Sql::getMaxValueOfField($App->tableCate, 'ordering', '') + 1;
            }
