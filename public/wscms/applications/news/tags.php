        ...
        if ($App->id > 0) {
            ...
            if ($delete == true && Core::$resultOp->error == 0) {
                ...
                if (Core::$resultOp->error == 0) {
                    Core::$resultOp->message = ucfirst((string) preg_replace('/%ITEM%/', (string) $_lang['tag'], (string) $_lang['%ITEM% cancellato'])).'!';
                }
            }
        }
        $App->viewMethod = 'list';
    ...
    case 'insertTags':
        if ($_POST) {
            ...
            if (!isset($_POST['ordering']) || (isset($_POST['ordering']) && $_POST['ordering'] == 0)) {
                $_POST['ordering'] = Sql::getMaxValueOfField($App->params->tables['tags'], 'ordering', '') + 1;
            }
...
switch ((string)$App->viewMethod) {
    case 'formNew':
        ...
        if (Core::$resultOp->error > 0) {
            Utilities::setItemDataObjWithPost($App->item, $App->params->fields['tags']);
        }
