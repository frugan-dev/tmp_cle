...
/*
    ...
    classes/class.Carts.php v.4.5.1. 23/07/2020
*/
...
class Carts extends Core
{
    ...
    public static function clearCartProducts()
    {
        ...
        Sql::deleteRecord();
        if (Core::$resultOp->error > 0) {
            die('errore db delete cart product');
            ToolsStrings::redirect(URL_SITE.'error/db');
        }
    }
    ...
    public static function deleteCart()
    {
        ...
        Sql::deleteRecord();
        if (Core::$resultOp->error > 0) {
            die('errore db cancellazione carrello');
        }
        self::$carts_id = 0;
        $_SESSION['carts_id'] = 0;
    }
    ...
    public static function delCartProduct($id)
    {
        ...
        Sql::deleteRecord();
        if (Core::$resultOp->error > 0) {
            die('Errore db cancellazione prodotto');
        }
    }
    ...
    public static function addCartProduct($id, $quantity = 1, $attributes_id = 0)
    {
        ...
        if ($id  > 0  && $quantity > 0) {
            ...
            $foo = Sql::getRecord();
            if (Core::$resultOp->error > 0) {
                die('Errore lettura prodotto per vedere se gia esistente');
            }
            ...
            if (isset($foo->id) && intval($foo->id) > 0) {
                ...
                Sql::updateRecord();
                if (Core::$resultOp->error > 0) {
                    die('Errore aggiornamento quantita prodotto gia esistente');
                }
            } else {
                ...
                Sql::insertRecord();
                if (Core::$resultOp->error > 0) {
                    die('Errore inserimento nuovo prodotto');
                }
