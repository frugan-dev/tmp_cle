 ...
 * classes/class.Multilanguage.php v.2.6.4. 30/07/2016
*/
...
class Multilanguage extends Core
{
    ...
    public static function getLocaleArrayValue($array, $field, $localeSuffix, $params)
    {
        ...
        if (isset($array[$rifString]) && $array[$rifString] != '') {
            $str = $array[$rifString];
        } elseif (isset($array[$rifStringDef])) {
    ...
    private static function getString($str, $opz)
    {
        ...
        if ($opz['parse'] == true) {
            $str = ToolsStrings::parseHtmlContent($str);
        }
        ...
        if ($opz['urlslug'] == true) {
            $str = SanitizeStrings::urlslug($str);
        }
        if ($opz['htmlawed'] == true) {
            $str = htmLawed::hl($str);
        }
        if ($opz['xss'] == true) {
            $str = SanitizeStrings::xss($str);
        }
