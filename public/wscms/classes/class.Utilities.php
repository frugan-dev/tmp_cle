...
class Utilities extends Core
{
    ...
    public static function getTitlesPage($moduledata, $pagedata, $lang, $opt)
    {
        ...
        if ($titles[$opt['titleField']] == '') {
            $titles[$opt['titleField']] = $moduledata;
        }
        if ($titles[$opt['titleField'].'Seo'] == '') {
            $titles[$opt['titleField'].'Seo'] = $moduledata;
        }
        if ($titles[$opt['titleField'].'Meta'] == '') {
            $titles[$opt['titleField'].'Meta'] = $moduledata;
        }
        if ($titles[$opt['titleField'].'Alt'] == '') {
            $titles[$opt['titleField'].'Alt'] = $moduledata;
        }
        if ($titles[$opt['titleField'].'Alt1'] == '') {
            $titles[$opt['titleField'].'Alt1'] = $moduledata;
        }
        return $titles;
    }
        ...
        if (is_array($array) && count($array) > 0) {
            ...
            foreach ($array as $value) {
                ...
                if (isset($value['title'])) {
                    $links = preg_replace('/%TITLE%/', (string) $value['title'], $links);
                }
                if (isset($value['url'])) {
                    $links = preg_replace('/%URL%/', (string) $value['url'], (string) $links);
                }
            }
    ...
    public static function getHTMLMessagesCore($obj, $opz = [])
    {
        ...
        if ($content != '') {
            ...
            if ($error == 1) {
                $alert = $opz['class danger'];
            }
            if ($error == 0) {
                $alert = $opz['class success'];
            }
            if ($error == 2) {
                $alert = $opz['class warning'];
            }
    ...
    public static function getPagination($page = 1, $itemsTotal = 1, $itemsForPage = 1)
    {
        ...
        if ($itemsForPage > 0) {
            $totalpage = ceil($itemsTotal / $itemsForPage);
        }
        if ($page > $totalpage) {
            $page = 1;
        }
        if ($itemsTotal >= $itemsForPage) {
            ...
            if ($lastPartItem > $itemsTotal) {
                $lastPartItem  = $itemsTotal;
            }
        }
        ...
        if (self::$resultOp->error == 0) {
            if (isset($itemData->$orderingFieldRif) && $itemData->$orderingFieldRif > 0) {
                ...
                if ($opt['parent'] == 1) {
                    ...
                    if (count($opt['addclauseparentvalues']) > 0) {
                        $fieldsValues = array_merge($fieldsValues, $opt['addclauseparentvalues']);
                    }
                    $where .= ' AND ' . $opt['parentField'] . ' = ?';
                    if ($opt['addclauseparent'] != '') {
                        $where .= ' AND ' . $opt['addclauseparent'];
                    }
                }
    ...
    public static function generatePagesTreeUlList($obj, $parent, $opz, $activePage = 1)
    {
            ...
            foreach ($obj as $key => $value) {
                ...
                if (intval($value->parent) == $parent) {
                    ...
                    if ($has_children === false) {
                        ...
                        if (self::$level > $opz['MainUl']) {
                            self::$treeResult .= '<ul' . $strShowUlId . ' class="' . $ulclass . '">' . "\n";
                        }
                    }
                    ...
                    if ($titleField != '') {
                        $fieldTitle = rtrim((string) $titleField, '_') . '_' . $langSuffix;
                    }
                    ...
                    if (self::$level > 0 && $value->sons > 0) {
                        $classLi = $classSubLiParent;
                    }
                    if (self::$level > 0 && $value->sons == 0) {
                        $classLi = $classDefLi;
                    }
                    /* active page */
                    if (self::$level == 0 && $value->alias == $activePage) {
                        $classLi .= ' active';
                    }
                    if (self::$level == 0) {
                        $classAref = $classMainAref;
                    }
                    if (self::$level == 0 && $value->sons == 0) {
                        $classAref = $classSubAref;
                    }
                    if (self::$level > 0 && $value->sons > 0) {
                        $classAref = $classSubArefParent;
                    }
                    if (self::$level == 0) {
                        $aRefSuffixString = $aRefSuffixStringMain;
                    }
                    if (self::$level == 0 && $value->sons == 0) {
                        $aRefSuffixString = $aRefSuffixStringSub;
                    }
                    if (self::$level > 0 && $value->sons > 0) {
                        $aRefSuffixString = $aRefSuffixStringSubParent;
                    }
                    if (self::$level == 0) {
                        $titlePrefixString = $titlePrefixStringMain;
                    }
                    if (self::$level == 0 && $value->sons == 0) {
                        $titlePrefixString = $titlePrefixStringSub;
                    }
                    if (self::$level > 0 && $value->sons > 0) {
                        $titlePrefixString = $titlePrefixStringSubParent;
                    }
                    if (self::$level > 0 && $value->sons == 0) {
                        $titlePrefixString = $titlePrefixStringSub;
                    }
                    if (self::$level == 0) {
                        $titleSuffixString = $titleSuffixStringMain;
                    }
                    if (self::$level == 0 && $value->sons == 0) {
                        $titleSuffixString = $titleSuffixStringSub;
                    }
                    if (self::$level > 0 && $value->sons > 0) {
                        $titleSuffixString = $titleSuffixStringSubParent;
                    }
                    if (self::$level > 0 && $value->sons == 0) {
                        $titleSuffixString = $titlePrefixStringSub;
                    }
        ...
        if ($has_children === true && self::$level > $opz['MainUl']) {
            self::$treeResult .= '</ul>' . "\n";
        }
    }
