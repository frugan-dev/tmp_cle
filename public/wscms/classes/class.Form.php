...
class Form extends Core
{
            ...
            foreach ($fields as $fieldName => $value) {
                ...
                if (!isset($_POST[$fieldName])) {
                    if (isset($value['defValue'])) {
                        $_POST[$fieldName] = $value['defValue'];
                    }
                }
                ...
                if ($opz['stripmagicfields'] == true && isset($_POST[$fieldName])) {
                    $_POST[$fieldName] = SanitizeStrings::stripMagic($_POST[$fieldName]);
                }
        ...
        switch ($foo[0]) {
            ...
            case 'float':
                ...
                if ($res == false) {
                    ...
                    if ($messaggio != '') {
                        Config::$resultOp->messages[$fieldName] = $messaggio;
                    }
                }
                break;
    ...
    public static function doFieldValidation($fieldName, $fieldDetails, $fieldPostValueArray, $fieldLabel, $fieldPostValue)
    {
        ...
        if ($result == false) {
            ...
            if (isset($fieldDetails['label'])) {
                $messaggio = preg_replace('/%ITEM%/', $fieldDetails['label'], (string) Config::$langVars['Il valore per il campo %ITEM% non è stato validato!']);
            }
            if ($returnmessage != '') {
                $messaggio = $returnmessage;
            }
            if (isset($fieldDetails['errorValidateMessage'])) {
                $messaggio = $fieldDetails['errorValidateMessage'];
            }
            if (isset($fieldDetails['error validate message'])) {
                $messaggio = $fieldDetails['error validate message'];
            }
            if ($messaggio != '') {
                Config::$resultOp->messages[$fieldName] = $messaggio;
            }
    ...
    public static function checkIfRequired($fieldName, $fieldDetails, $fieldPostValue)
    {
        if (!isset($fieldPostValue[$fieldName]) || (isset($fieldPostValue[$fieldName]) && $fieldPostValue[$fieldName] == '')) {
            ...
            if (isset($fieldDetails['label'])) {
                $messaggio = preg_replace('/%ITEM%/', $fieldDetails['label'], (string) Config::$langVars['Devi inserire il campo %ITEM%!']);
            }
            if (isset($fieldDetails['errorMessage'])) {
                $messaggio = $fieldDetails['errorMessage'];
            }
            if (isset($fieldDetails['error message'])) {
                $messaggio = $fieldDetails['error message'];
            }
            if ($messaggio != '') {
                self::$resultOp->messages[$fieldName] = $messaggio;
            }
        }
    }
