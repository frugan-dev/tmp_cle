...
class Core extends Config
{
    ...
    public static function getRequest()
    {
        ...
        if (isset($_POST['action']) && $_POST['action'] != '') {
            self::$request->action = $_POST['action'];
        }
        if (isset($_POST['method']) && $_POST['method'] != '') {
            self::$request->method = $_POST['method'];
        }
        if (isset($_POST['id']) && $_POST['id'] != '') {
            self::$request->param = intval($_POST['id']);
        }
        if (isset($_POST['param']) && $_POST['param'] != '') {
            self::$request->param = $_POST['param'];
        }
        if (isset($_POST['pages']) && $_POST['pages'] != '') {
            self::$request->page = $_POST['page'];
        }
        if (isset($_POST['lang']) && $_POST['lang'] != '') {
            self::$request->lang = $_POST['lang'];
        }
        ...
        if (count(self::$request->params) > 0) {
            foreach (self::$request->params as $key => $value) {
                if (isset(self::$request->params[$key])) {
                    self::$request->params[$key] = SanitizeStrings::xssClean(self::$request->params[$key]);
                }
            }
        }
    }
    ...
    public static function parseInitReqs($parts)
    {
        ...
        if (Core::$globalSettings['requestoption']['isRoot'] == 1) {
            $userLoggedData->is_root = 1;
        }
        ...
        if (isset($action) && $action != '') {
            ...
            if ($checkroute == false && isset(Core::$globalSettings['requestoption']['getactionfrommodulefields']) && Core::$globalSettings['requestoption']['getactionfrommodulefields'] != '') {
                ...
                if ($key != '') {
                    ...
                    if (isset($parts[0])) {
                        ...
                        if (isset($parts[1])) {
                            Core::$request->param_alias = $parts[1];
                        }
                    }
                    if (isset($parts[0])) {
                        $parts[0] = $module->name;
            ...
            if ($checkroute == false && array_key_exists($action, Config::$userModules)) {
                ...
                if (isset(Core::$globalSettings['requestoption']['getactionfrommodulefields']) && Core::$globalSettings['requestoption']['getactionfrommodulefields'] != '') {
                    $field = Core::$globalSettings['requestoption']['getactionfrommodulefields'];
                    if (isset($parts[0])) {
                        $parts[0] = Config::$userModules[$action]->name;
                    }
    ...
    public static function oldparseInitReqs($parts, $opz)
    {
        ...
        if (isset($action) && $action != '') {
            ...
            if ($opz['managechangeaction'] == 1 && $changeaction == true) {
                ...
                if (Core::$resultOp->error == 1) {
                    die('Errore db lettura pagina!');
                }
                if (isset(Core::$request->page_data->alias)) {
                    Core::$request->page_alias = Core::$request->page_data->alias;
                }
                if (isset(Core::$request->page_data->id)) {
                    Core::$request->page_id = Core::$request->page_id;
                }
            } else {
    ...
    public static function getRequestParam($param)
    {
        $paramValue = '';
        /* se è un method */
        if (self::$request->method === $param && isset(self::$request->param)) {
            $paramValue = self::$request->param;
        }
        /* se è un param */
        if (self::$request->param === $param && isset(self::$request->params[0])) {
            $paramValue = self::$request->params[0];
        }
        /* se è un params */
