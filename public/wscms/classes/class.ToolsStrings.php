...
class ToolsStrings extends Core
{
    ...
    public static function redirect($url)
    {
        ...
        if ($server_name != '') {
            ...
            } elseif (!preg_match('#^[a-z]+://#', (string) $url)) {
    ...
    {
        if ($alias == '') {
            $alias = SanitizeStrings::getAliasString($value, []);
        }
        $aliascheck = false;
        do {
    ...
    public static function getStringFromTotNumberChar($str, $opz)
    {
        ...
        if (strlen($str) > $opz['numchars']) {
            $str = mb_strcut($str, 0, $opz['numchars']).$opz['suffix'];
        }
        return $str;
    }
    ...
    public static function findValueInArrayWithObject($arrayobject, $rifobject, $rifvalue, $opt)
    {
        ...
        if (is_array($arrayobject) && $rifobject != '' && $rifvalue != '') {
            foreach ($arrayobject as $key => $value) {
                if (isset($value->$rifobject) && $value->$rifobject == $rifvalue) {
                    $result = true;
                }
            }
        }
    ...
    public static function parseHtmlContent($str, $opz = [])
    {
        ...
        if (isset(Config::$globalSettings['azienda stato'])) {
            $str = preg_replace('/%AZIENDASTATO%/', Config::$globalSettings['azienda stato'], (string) $str);
        }
        ...
        if (isset(Config::$globalSettings['azienda mobile'])) {
            $str = preg_replace('/%AZIENDAMOBILE%/', Config::$globalSettings['azienda mobile'], (string) $str);
        }
