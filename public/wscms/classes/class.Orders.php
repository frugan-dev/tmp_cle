...
class Orders extends Core
{
    ...
    */
    ...
    public static function getOrderProducts($id)
    {
        ...
        if ($id > 0) {
            ...
            if (Core::$resultOp->error > 0) {
                die('errore db lettura prodotto carrello');
            }
    ...
    public static function getOrdersList()
    {
        ...
        if (self::$whereDbQueryClause != '') {
            $where .= $and . self::$whereDbQueryClause;
        }
        if (self::$whereAndDbQueryClause != '') {
            $and = self::$whereAndDbQueryClause;
        }
    ...
    */
    public static function sendOrderStaffEmail($textHtml, $user, $company, $order, $new = 1)
    {
        ...
        if (isset($company->email1) && $company->email1 != '') {
            $address[] = $company->email1;
        }
        if (isset($company->email2) && $company->email2 != '') {
            $address[] = $company->email2;
        }
        if (isset($address[0])) {
            $email = $address[0];
            unset($address[0]);
    ...
    public static function getHtmlInvoice73x()
    {
        ...
        if ($dati->company->location_comuni_id == 0) {
            $dati->company->comune = $dati->company->comune_alt;
        }
        if ($dati->company->location_province_id == 0) {
            $dati->company->provincia = $dati->company->provincia_alt;
        }

        if ($dati->user->location_comuni_id == 0) {
            $dati->user->comune = $dati->user->comune_alt;
        }
        if ($dati->user->location_province_id == 0) {
            $dati->user->provincia = $dati->user->provincia_alt;
        }
        /*
        $html = <<<HTML
        <!DOCTYPE html>
