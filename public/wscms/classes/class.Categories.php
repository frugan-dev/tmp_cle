...
class Categories extends Core
{
    ...
    public static function generateCategoriesTreeUlList($obj, $parent, $opt, $activePage = 1)
    {
        ...
        if ($has_children === true && self::$level > $opt['MainUl']) {
            self::$treeResult .= '</ul>'."\n";
        }
    }
    ...
    {
        $optDef = ['type' => 1,'multilanguage' => 1,'ordering' => 1,'active' => 1];
		...
		c.title AS title,';
        if ($imageField == true) {
            $qry .= 'c.filename AS filename,c.org_filename AS org_filename,';
        }
        if ($opt['ordering'] == 1) {
        ...
        if ($countItems == true) {
            $qry .= '(SELECT COUNT(i.id) FROM '.$tableItem.' AS i WHERE i.id_cat = c.id) AS items,';
        }
        ...
        if ($opt['ordering'] == 1) {
            $qry .= ' ORDER BY ordering DESC';
        }

        if (isset($opt['qry']) && $opt['qry'] != '') {
            $qry = $opt['qry'];
        }
        $obj = '';
        Sql::resetListDataVar();
        $obj = Sql::getListParentData($qry, [], $initParent, $opt);
        return $obj;
    }
    ...
    {
        $obj =  new stdClass();
        $findOne = ($opt['findOne'] ?? true);
        $actived = ($opt['actived'] ?? true);
