<!-- wscms/sponsor/listItem.tpl.php v.1.0.0. 28/05/2016 -->
<div class="row">
	...
	<div class="card-body">
		<form role="form" action="{{ URLSITEADMIN }}{{ CoreRequest.action }}" method="post" enctype="multipart/form-data">
			...
			<!-- <div class="form-group row">
				<label for="categories_idID" class="col-sm-12 col-md-4 col-lg-3 col-xl-2 col-form-label">{{ Lang['categoria']|capitalize }}</label>
				<div class="col-sm-12 col-md-6 col-lg-3 col-xl-4">
				
					<select class="form-control input-md" name="id_cat" onchange="this.form.submit();">
						<option 
						value="0"
						{% if App.id_cat == 0 %} selected="selected"{% endif %}
						>Tutte</option>

							{% if App.categoriesData is iterable and App.categoriesData|length > 0 %}	
								{% for key,value in App.categoriesData %}											
									<option value="{{ value.id }}" {% if App.id_cat is defined and App.id_cat == value.id  %}selected="selected" {% endif %}>{{ value.title_it|e(html) }}
								</option>
						{% endfor %}
						{% endif %}
					</select>
						...
						{% if App.items is iterable and App.items|length > 0 %}
							{% for key,value in App.items %}
								...
								<tr>
									...
									<td>
										{% if value.filename is defined and value.filename != '' %}
											<a class="" href="{{ App.itemUploadDir }}{{ value.filename }}" data-lightbox="image-1" title="{{ value.org_filename }}">
												<img class="img-miniature" src="{{ App.itemUploadDir }}{{ value.filename }}" alt="{{ value.org_filename }}">
											</a>
										{% else %}	
											<img class="img-miniature" src="{{ App.itemUploadDir }}default/image.png" alt="Immagine di default">
										{% endif %}	
									</td>
									
									<!-- <td>
										<a class="btn btn-default btn-circle" href="{{ URLSITE }}{{ CoreRequest.action }}/listIimg/{{ value.id }}" title="Immagini associate"><i class="fa fa-picture-o"> </i></a>
										&nbsp;({{ value.images }})
									</td> -->
									...
									<td class="ordering">
										...
										{% endif %}							
										<a class="" href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/{{ App.params.orderingType == 'DESC' ? 'less' : 'more' }}OrderingItem/{{ value.id }}" title="{{ Lang['sposta']|capitalize }} {{ App.params.orderingType == 'DESC' ? Lang['su'] : Lang['giu'] }}"><i class="fas fa-long-arrow-alt-down"></i></a>
										<a class="" href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/{{ App.params.orderingType == 'DESC' ? 'more' : 'less' }}OrderingItem/{{ value.id }}" title="{{ Lang['sposta']|capitalize }} {{ App.params.orderingType == 'DESC' ? Lang['giu'] : Lang['su'] }}"><i class="fas fa-long-arrow-alt-up"></i></a>								
									</td>			
								
									<!-- <td>
										<a class="btn btn-default btn-circle" href="{{ URLSITE }}{{ CoreRequest.action }}/listIfil/{{ value.id }}" title="File associati"><i class="fa fa-file-o"> </i> </a>
										&nbsp;({{ value.files }})
									</td> -->
							
									<td class="actions">
										
										<a class="btn btn-default btn-sm" href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/{{ value.active == 1 ? 'disactive' : 'active' }}Item/{{ value.id  }}" title="{{ value.active == 1 ? Lang['disattiva']|capitalize : Lang['attiva']|capitalize }} {{ Lang['la voce'] }}"><i class="fa fa-{{ value.active == 1 ? 'unlock' : 'lock' }}"></i></a>			 
										<a class="btn btn-default btn-sm" href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/modifyItem/{{ value.id }}" title="{{ Lang['modifica']|capitalize }} {{ Lang['la voce'] }}"><i class="far fa-edit"></i></a>
										<a class="btn btn-default btn-sm confirm" href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/deleteItem/{{ value.id }}" title="{{ Lang['cancella']|capitalize }} {{ Lang['la voce'] }}"><i class="far fa-trash-alt"></i></a>
									</td>		
								</tr>
							{% endfor %}
						{% else %}
