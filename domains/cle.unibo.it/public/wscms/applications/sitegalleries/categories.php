		   	...
		   	if ($count == 0) {
			   	...
			   	if (Core::$resultOp->error == 0) { 	
			   		...
			   		if (Core::$resultOp->error == 0) {
						...
						Core::$resultOp->message = 'Esiste già un'.$App->labels['cate']['itemSex'].' '.$App->labels['cate']['item'].' con lo stesso nome!';	
		   		...
		   		if ($oldCate_name == $newCate_name || ($oldCate_name != $newCate_name && $count == 0)) {
			   		...
			   		if (Core::$resultOp->error == 0) {   	
			   			...
			   			if (Core::$resultOp->error == 0) {	   			
			   				...
			   				if ($oldCate_name != '') {
			   					if($oldCate_name != '' && file_exists ($App->itemUploadPathDir.$oldCate_name)) rename($App->itemUploadPathDir.$oldCate_name,$App->itemUploadPathDir.$newCate_name) or die('impossibile rinominare la cartella');
			   					} 			   					
			   			...
			   			Core::$resultOp->messages[] = 'Esiste già un'.$App->labels['cate']['itemSex'].' '.$App->labels['cate']['item'].' con lo stesso nome!';	
							Core::$resultOp->error = 1;
							}
					}
				}						
