...
switch(Core::$request->method) {
	case 'moreOrderingIfil':
		...
		Utilities::increaseFieldOrdering($App->id,$_lang,array('table'=>$App->params->tables['ifil'],'orderingType'=>$App->params->orderTypes['ifil'],'parent'=>1,'parentField'=>'id_owner','label'=>ucfirst($_lang['file']).' '.$_lang['spostato']));
		$_SESSION['message'] = '0|'.Core::$resultOp->message;
		ToolsStrings::redirect(URL_SITE_ADMIN.Core::$request->action.'/listIfil');
	break;
	case 'lessOrderingIfil':
		if ($App->id == 0) { ToolsStrings::redirect(URL_SITE_ADMIN.'error/404'); }
		Utilities::decreaseFieldOrdering($App->id,$_lang,array('table'=>$App->params->tables['ifil'],'orderingType'=>$App->params->orderTypes['ifil'],'parent'=>1,'parentField'=>'id_owner','label'=>ucfirst($_lang['file']).' '.$_lang['spostato']));
		$_SESSION['message'] = '0|'.Core::$resultOp->message;
		ToolsStrings::redirect(URL_SITE_ADMIN.Core::$request->action.'/listIfil');
	break;
	...
	case 'insertIfil':
		...
		if (!isset($_POST['ordering']) || (isset($_POST['ordering']) && intval($_POST['ordering']) == 0)) $_POST['ordering'] = Sql::getMaxValueOfField($App->params->tables['item resources'],'ordering','id_owner = '.intval($_POST['id_owner']).' AND resource_type = 2') + 1;
