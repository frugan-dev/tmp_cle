<?php
/* wscms/pages/block-images.php v.3.5.1. 10/01/2018 */
...
if (Core::$request->method == 'listBimg' && $App->id > 0) $_MY_SESSION_VARS = $my_session->addSessionsModuleSingleVar($_MY_SESSION_VARS,$App->sessionName,'id_owner',$App->id);
...
if (!isset($App->ownerData->id) || (isset($App->ownerData->id) && $App->ownerData->id == 0)) {
	ToolsStrings::redirect(URL_SITE_ADMIN.Core::$request->action.'/messageIblo/2/'.urlencode($_lang['Devi creare o attivare almeno una voce!']));
	die();
	}
...
switch(Core::$request->method) {
	case 'moreOrderingBimg':
		Utilities::increaseFieldOrdering($App->id,$_lang,array('table'=>$App->params->tables['resources'],'orderingType'=>$App->params->ordersType['bimg'],'parent'=>1,'parentField'=>'id_owner','label'=>ucfirst($_lang['immagine']).' '.$_lang['spostata'],'addclauseparent'=>'resource_type = ?','addclauseparentvalues'=>array(1)));
		$App->viewMethod = 'list';	
	break;
	case 'lessOrderingBimg':
		Utilities::decreaseFieldOrdering($App->id,$_lang,array('table'=>$App->params->tables['resources'],'orderingType'=>$App->params->ordersType['bimg'],'parent'=>1,'parentField'=>'id_owner','label'=>ucfirst($_lang['immagine']).' '.$_lang['spostata'],'addclauseparent'=>'resource_type = ?','addclauseparentvalues'=>array(1)));
		$App->viewMethod = 'list';		
	break;

	case 'activeBimg':
	case 'disactiveBimg':
		Sql::manageFieldActive(substr(Core::$request->method,0,-4),$App->params->tables['resources'],$App->id,array('label'=>$_lang['immagine'],'attivata'=>$_lang['attivata'],'disattivata'=>$_lang['disattivata']));
		$App->viewMethod = 'list';		
	break;
		...
		if ($App->id > 0) { 
		   ...
		   if (Core::$resultOp->error == 0) {
				...
				if (Core::$resultOp->error == 0) {
					...
					if (file_exists($App->params->uploadPaths['bimg'].$App->itemOld->filename)) {			
						@unlink($App->params->uploadPaths['bimg'].$App->itemOld->filename);			
						} 			
					Core::$resultOp->message = ucfirst($_lang['immagine cancellata']).'!';		
					}
				}
			}
		$App->viewMethod = 'list';	
	   ...
	   if ($_POST) {
	   	...
	   	if (Core::$resultOp->error == 0) {   	
		   	...
		   	if (Core::$resultOp->error == 0) {	   	 		
		   		...
		   		if (Core::$resultOp->error == 0) {	   	 		
			   	   /* sposto il Bimg */
			   		if ($_POST['filename'] != '') {
			   			move_uploaded_file(ToolsUpload::getTempFilename(),$App->params->uploadPaths['bimg'].$_POST['filename']) or die('Errore caricamento file');
			   			}
			   		}
					}
				}
		...
		if (Core::$resultOp->error > 0) {
			...
			$App->viewMethod = 'formNew';
				...
				$App->viewMethod = 'list';
				Core::$resultOp->message = ucfirst($_lang['immagine inserita']).'!';			
				}
	break;
	   		...
	   		if (Core::$resultOp->error == 0) {	   		  		   	
			   	...
			   	$_POST['type'] = ToolsUpload::getFileType();  
					...
					if (Core::$resultOp->error == 0) {
						...
						if (Core::$resultOp->error == 0) {   	
							if ($uploadFilename != '') {
					   		move_uploaded_file(ToolsUpload::getTempFilename(),$App->params->uploadPaths['bimg'].$uploadFilename) or die('Errore caricamento file');   			
		...
		if (Core::$resultOp->error == 1) {
			...
			$App->viewMethod = 'formMod';	
				...
				if (isset($_POST['submitForm'])) {	
					$App->viewMethod = 'list';
					Core::$resultOp->message = ucfirst($_lang['immagine modificata']).'!';								
					} else {						
						if (isset($_POST['id'])) {
							$App->id = $_POST['id'];
							$App->pageSubTitle .= $_lang['modifica immagine'];
	...
	case 'downloadBimg':
		if($App->id > 0) {	
			$renderTpl = false;		
			ToolsUpload::downloadFileFromDB($App->params->uploadPaths['bimg'],$App->params->tables['resources'],$App->id,'filename','org_filename','','');	
			die();
			}
		$App->viewMethod = 'list';
	break;
...
switch((string)$App->viewMethod){
	...
	case 'formNew':
		...
		$App->templateApp = 'formBimg.tpl.php';
		$App->methodForm = 'insertBimg';
		$App->jscript[] = '<script src="'.URL_SITE_ADMIN.$App->pathApplication.Core::$request->action.'/templates/'.$App->templateUser.'/js/formBimg.js"></script>';
	break;
	...
	case 'list':
		...
		Sql::setOrder('ordering '.$App->params->ordersType['bimg']);
		...
		$App->templateApp = 'listBimg.tpl.php';	
		$App->jscript[] = '<script src="'.URL_SITE_ADMIN.$App->pathApplication.Core::$request->action.'/templates/'.$App->templateUser.'/js/listBimg.js"></script>';		
	break;		
