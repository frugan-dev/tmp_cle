...
switch(Core::$request->method) {
	case 'moreOrderingIimg':
		Utilities::increaseFieldOrdering($App->id,$_lang,array('table'=>$App->params->tables['resources'],'orderingType'=>$App->params->ordersType['iimg'],'parent'=>1,'parentField'=>'id_owner','label'=>ucfirst($_lang['immagine']).' '.$_lang['spostata'],'addclauseparent'=>'resource_type = ?','addclauseparentvalues'=>array(1)));
		$App->viewMethod = 'list';	
	break;
	case 'lessOrderingIimg':
		Utilities::decreaseFieldOrdering($App->id,$_lang,array('table'=>$App->params->tables['resources'],'orderingType'=>$App->params->ordersType['iimg'],'parent'=>1,'parentField'=>'id_owner','label'=>ucfirst($_lang['immagine']).' '.$_lang['spostata'],'addclauseparent'=>'resource_type = ?','addclauseparentvalues'=>array(1)));
		$App->viewMethod = 'list';		
	break;
	...
	case 'newIimg':			
		$App->pageSubTitle = preg_replace('/%ITEM%/',Core::$langVars['immagine'],Core::$langVars['inserisci %ITEM%']);
	...
	case 'insertIimg':
	   if ($_POST) {
	   	...
	   	if (!isset($_POST['ordering']) || (isset($_POST['ordering']) && intval($_POST['ordering']) == 0)) $_POST['ordering'] = Sql::getMaxValueOfField($App->params->tables['resources'],'ordering','id_owner = '.intval($_POST['id_owner']).' AND resource_type = 1') + 1;
	   	...
	   	if (Core::$resultOp->error == 0) {   	
		   	...
		   	$_POST['type'] = ToolsUpload::getFileType();  
				...
				Form::parsePostByFields($App->params->fields['resources'],$_lang,array());
		...
		if (Core::$resultOp->error > 0) {
			$App->pageSubTitle .= 'inserisci immagine';
			$App->viewMethod = 'formNew';
			} else {
				$App->viewMethod = 'list';
				Core::$resultOp->message = 'Immagine inserita!';			
				}
	break;

	case 'modifyIimg':		
		$App->pageSubTitle = preg_replace('/%ITEM%/',Core::$langVars['immagine'],Core::$langVars['modifica %ITEM%']);
		...
		if (Core::$resultOp->error == 1) {
			...
			$App->viewMethod = 'formMod';	
				...
				if (isset($_POST['submitForm'])) {	
					$App->viewMethod = 'list';
					Core::$resultOp->message = 'Immagine modificata!';								
					} else {						
						if (isset($_POST['id'])) {
							$App->id = $_POST['id'];
							$App->pageSubTitle .= 'modifica immagine';
							$App->viewMethod = 'formMod';	
...
switch((string)$App->viewMethod){
	...
	case 'formNew':
		...
		$App->jscript[] = '<script src="'.URL_SITE_ADMIN.$App->pathApplications.Core::$request->action.'/templates/'.$App->templateUser.'/js/formIimg.js"></script>';
	break;
	...
	case 'list':
		...
		Sql::setOrder('ordering '.$App->params->orderTypes['iimg']);
		...
		$App->pageSubTitle .= 'lista delle immagini';
		$App->templateApp = 'listIimg.tpl.php';	
		$App->jscript[] = '<script src="'.URL_SITE_ADMIN.$App->pathApplications.Core::$request->action.'/templates/'.$App->templateUser.'/js/listIimg.js"></script>';		
	break;		
