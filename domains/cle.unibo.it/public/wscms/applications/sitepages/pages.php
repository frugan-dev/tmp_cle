	...
	case 'insert':
		if ($_POST) {
			...
			foreach ($globalSettings['languages'] AS $lang){
				if ($_POST['title_seo_'.$lang] == '') $_POST['title_seo_'.$lang] = SanitizeStrings::cleanTitleUrl($_POST['title_'.$lang]);
				if ($_POST['title_meta_'.$lang] == '') $_POST['title_meta_'.$lang] = SanitizeStrings::cleanTitleUrl($_POST['title_'.$lang]);
				}	
	   	...
	   	/* imposta alias */
			...
			if(Core::$resultOp->error == 0) {
				...
				if(Core::$resultOp->error == 0) {
					...
					if ($App->templateItem->contents_html > 0) $Module->updatePageContents($tableRif,$App->id,$App->templateItem->contents_html,$globalSettings['languages']);	
					/* images */
					if ($App->templateItem->images > 0) $Module->updatePageItems($tableRif,$App->id,'pageImages');
					/* file */
					if ($App->templateItem->files > 0) $Module->updatePageItems($tableRif,$App->id,'pageFiles');
					/* gallerie */
					if ($App->templateItem->galleries > 0) $Module->updatePageItems($tableRif,$App->id,'pageGalleries');
					/* blocks */
					if ($App->templateItem->blocks > 0) $Module->updatePageItems($tableRif,$App->id,'pageBlocks');	   			
					}
				}	
			} else {					
				Core::$resultOp->error = 1;
