...
switch(Core::$request->method) {
	case 'moreOrderingIfil':
		$Utilities::increaseFieldOrdering($App->id,array('table'=>$App->tableIfil,'orderingType'=>$App->orderingType,'parent'=>true,'parentField'=>'id_owner','sexSuffix'=>$App->labels['ifil']['itemSex'],'labelItem'=>ucfirst($App->labels['ifil']['item'])));
		$App->viewMethod = 'list';	
	break;
	case 'lessOrderingIfil':
		$Utilities::decreaseFieldOrdering($App->id,array('table'=>$App->tableIfil,'orderingType'=>$App->orderingType,'parent'=>true,'parentField'=>'id_owner','sexSuffix'=>$App->labels['ifil']['itemSex'],'labelItem'=>ucfirst($App->labels['ifil']['item'])));
		$App->viewMethod = 'list';		
	break;
