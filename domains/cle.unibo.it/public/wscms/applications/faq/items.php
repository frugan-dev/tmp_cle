...
switch(Core::$request->method) {	
	...
	case 'deleteItem':
		if ($App->id > 0) {
			...
			$_SESSION['message'] = '0|'.ucfirst(preg_replace('/%ITEM%/',(string) $_lang['voce'],(string) $_lang['%ITEM% cancellata'])).'!';	
			ToolsStrings::redirect(URL_SITE_ADMIN.Core::$request->action.'/listItem');		
		} else {
	...
	case 'insertItem':
		...
		if ($_POST) {	
			...
			$_SESSION['message'] = '0|'.ucfirst(preg_replace('/%ITEM%/',(string) $_lang['voce'],(string) $_lang['%ITEM% inserita']));
			ToolsStrings::redirect(URL_SITE_ADMIN.Core::$request->action.'/listItem');				
	...
	case 'updateItem':
		if ($_POST) {
			...
			if (Core::$resultOp->error == 0) {
				...
				$_SESSION['message'] = '0|'.ucfirst(preg_replace('/%ITEM%/',(string) $_lang['voce'],(string) $_lang['%ITEM% modificata']));
