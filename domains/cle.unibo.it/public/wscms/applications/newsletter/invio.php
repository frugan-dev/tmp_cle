...
switch(Core::$request->method) {	
	...
	case 'ajaxMoveAddressToSendList':
		if (isset($_POST['listAddress'])) {
			...
			foreach($arr AS $keyAddress) {
				...
				$obj = new stdClass();
				Sql::initQuery($App->tableInd,array('*'),array($keyAddress),'active = 1 AND id = ?');
				$obj = Sql::getRecord();
				if (Core::$resultOp->error > 0) { die('Errore database'); ToolsStrings::redirect(URL_SITE_ADMIN.'error/db'); }
				...
				if (isset($obj->email) || $obj->email != '') {	
					...
					Sql::initQuery($App->tableIndInvio,array('*'),array($obj->email),'email = ?');
					$foo = Sql::getRecords();	
					if (Core::$resultOp->error > 0) { die('Errore database'); ToolsStrings::redirect(URL_SITE_ADMIN.'error/db'); }
					...
					if ($count == 0) {
						...
						Sql::initQuery($App->tableIndInvio,array('email','hash','inviata'),array($obj->email,$obj->hash,'0'));
						Sql::insertRecord();
						if (Core::$resultOp->error > 0) { die('Errore database'); ToolsStrings::redirect(URL_SITE_ADMIN.'error/db'); }
					}				
				}
	...
	default;
		...
		if (Core::$resultOp->error == 0) {
			...
			Sql::initQuery($App->tableInd,array('*'));
			$clause = 'active = 1';
