...
<div class="row">
	<div class="col-md-3 new">
		<a href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/newCate" title="{{ Lang['inserisci una nuova %ITEM%']|replace({'%ITEM%':Lang['categoria'] })|capitalize }}" class="btn btn-sm btn-primary">{{ Lang['nuova %ITEM%']|replace({'%ITEM%':Lang['categoria'] })|capitalize }}</a>
	</div>
							...
							{% if App.items is iterable and App.items|length > 0 %}
								{% for key,value in App.items %}
									<tr>										
										...
										{% endif %}							
										<a class="" href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/{{ App.params.orderingType == 'DESC' ? 'less' : 'more' }}OrderingCate/{{ value.id }}" title="{{ Lang['sposta']|capitalize }} {{ App.params.orderingType == 'DESC' ? Lang['su'] : Lang['giu'] }}"><i class="fas fa-long-arrow-alt-down"></i></a>
										<a class="" href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/{{ App.params.orderingType == 'DESC' ? 'more' : 'less' }}OrderingCate/{{ value.id }}" title="{{ Lang['sposta']|capitalize }} {{ App.params.orderingType == 'DESC' ? Lang['giu'] : Lang['su'] }}"><i class="fas fa-long-arrow-alt-up"></i></a>								
									</td>	
									<td><a href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/listItem/{{ value.id }}" title="Vai ai contenuti della pagina"><i class="fas fa-newspaper"></i> - {{ value.numitems }}</a></td>
								
									<td class="actions">
										<a class="btn btn-default btn-sm" href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/{{ value.active == 1 ? 'disactive' : 'active' }}Cate/{{ value.id  }}" title="{{ value.active == 1 ? Lang['disattiva']|capitalize : Lang['attiva']|capitalize }} {{ Lang['categoria'] }}"><i class="fa fa-{{ value.active == 1 ? 'unlock' : 'lock' }}"></i></a>			 
										<a class="btn btn-default btn-sm" href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/modifyCate/{{ value.id }}" title="{{ Lang['modifica']|capitalize }} {{ Lang['categoria'] }}"><i class="far fa-edit"></i></a>
										<a class="btn btn-default btn-sm confirm" href="{{ URLSITEADMIN }}{{ CoreRequest.action }}/deleteCate/{{ value.id }}" title="{{ Lang['cancella']|capitalize }} {{ Lang['categoria'] }}"><i class="far fa-trash-alt"></i></a>
									</td>						
									{% endfor %}
						{% else %}
							<tr>
