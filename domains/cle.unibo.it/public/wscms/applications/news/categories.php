...
switch(Core::$request->method) {
	...
	case 'activeCate':
	case 'disactiveCate':
		Sql::manageFieldActive(substr(Core::$request->method,0,-4),$App->params->tableCate,$App->id,array('label'=>$_lang['categoria'],'attivata'=>$_lang['attivata'],'disattivata'=>$_lang['disattivata']));
	...
	case 'moreOrderingCate':
		Utilities::increaseFieldOrdering($App->id,$_lang,array('table'=>$App->params->tableCate,'orderingType'=>$App->params->orderingType,'parent'=>0,'parentField'=>'','label'=>ucfirst($_lang['categoria']).' '.$_lang['spostata']));
		$_SESSION['message'] = '0|'.Core::$resultOp->message;
		ToolsStrings::redirect(URL_SITE_ADMIN.Core::$request->action.'/listCate');
	break;
	case 'lessOrderingCate':
		Utilities::decreaseFieldOrdering($App->id,$_lang,array('table'=>$App->params->tableCate,'orderingType'=>$App->params->orderingType,'parent'=>0,'parentField'=>'','label'=>ucfirst($_lang['categoria']).' '.$_lang['spostata']));
		$_SESSION['message'] = '0|'.Core::$resultOp->message;
		ToolsStrings::redirect(URL_SITE_ADMIN.Core::$request->action.'/listCate');
	break;
