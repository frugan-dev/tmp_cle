...
switch(Core::$request->method) {
	...
	case 'activeFold':
	case 'disactiveFold':
		Sql::manageFieldActive(substr((string) Core::$request->method,0,-4),$App->params->tableFold,$App->id,['label'=>$_lang['categoria'],'attivata'=>$_lang['attivata'],'disattivata'=>$_lang['disattivata']]);
	...
	case 'deleteFold':
		if ($App->id > 0) {
			...
			Sql::initQuery($App->params->tableItem,['id'],[$App->id],'id_folder = ?');
			...
			Sql::initQuery($App->params->tableFold,['*'],[$App->id],'id = ?');
			$App->itemOld = Sql::getRecord();
			if (Core::$resultOp->error > 0) { ToolsStrings::redirect(URL_SITE.'error/db'); }	

			Sql::initQuery($App->params->tableFold,['id'],[$App->id],'id = ?');
			Sql::deleteRecord();
			if (Core::$resultOp->error > 0) { ToolsStrings::redirect(URL_SITE.'error/db'); }
	...
	case 'insertFold':
	   if ($_POST) {
	   	...
	   	Sql::initQuery($App->params->tableFold,['id'],[$folder_name],'folder_name = ?');
			$count = Sql::countRecord();
		...
		if ($_POST) {
		   ...
		   $newFold_name = SanitizeStrings::cleanTitleUrl($_POST['title_it']);
			...
			if (Core::$resultOp->error == 0) {		
				...
				/* controlla se esiste già una cartella con lo stesso nome */
				Sql::initQuery($App->params->tableFold,['id'],[$newFold_name],'folder_name = ?');
				$count = Sql::countRecord();	
				if (Core::$resultOp->error == 0) {	
