...
switch(Core::$request->method) {
	...
	case 'activeFold':
	case 'disactiveFold':
		Sql::manageFieldActive(substr(Core::$request->method,0,-4),$App->params->tableFold,$App->id,array('label'=>$_lang['categoria'],'attivata'=>$_lang['attivata'],'disattivata'=>$_lang['disattivata']));
	...
	case 'deleteFold':
		if ($App->id > 0) {
			...
			Sql::initQuery($App->params->tableItem,array('id'),array($App->id),'id_folder = ?');
			...
			Sql::initQuery($App->params->tableFold,array('*'),array($App->id),'id = ?');
			$App->itemOld = Sql::getRecord();
			if (Core::$resultOp->error > 0) { ToolsStrings::redirect(URL_SITE.'error/db'); }	

			Sql::initQuery($App->params->tableFold,array('id'),array($App->id),'id = ?');
			Sql::deleteRecord();
			if (Core::$resultOp->error > 0) { ToolsStrings::redirect(URL_SITE.'error/db'); }
	...
	case 'insertFold':
	   if ($_POST) {
	   	...
	   	Sql::initQuery($App->params->tableFold,array('id'),array($folder_name),'folder_name = ?');
			$count = Sql::countRecord();
			if (Core::$resultOp->error == 0) { 	
		   	if ($count == 0) {
			   	...
			   	Sql::checkRequireFields($App->params->fieldsFold);
			   	if (Core::$resultOp->error == 0) { 	
			   		Sql::stripMagicFields($_POST);
			   		/* memorizza nel db */
			   		Sql::insertRawlyPost($App->params->fieldsFold,$App->params->tableFold);
			   		if (Core::$resultOp->error == 0) {
			   			/* crea la cartella galleria */		   			
			   			if(!file_exists ($App->itemUploadPathDir.$folder_name)) mkdir($App->itemUploadPathDir.$folder_name) or die('impossibile creare la cartella'.$App->itemUploadPathDir.$folder_name);
		...
		if (Core::$resultOp->error > 0) {
			...
			$App->viewMethod = 'formNew';
				...
				$App->viewMethod = 'list';
				Core::$resultOp->message = ucfirst($App->labels['fold']['item']).' inserit'.$App->labels['fold']['itemSex'].'!';				
				}		
	break;
		...
		if ($_POST) {
		   ...
		   $newFold_name = SanitizeStrings::cleanTitleUrl($_POST['title_it']);
			...
			if (Core::$resultOp->error == 0) {		
				...
				/* controlla se esiste già una cartella con lo stesso nome */
				Sql::initQuery($App->params->tableFold,array('id'),array($newFold_name),'folder_name = ?');
				$count = Sql::countRecord();	
				if (Core::$resultOp->error == 0) {	
		   		if ($oldFold_name == $newFold_name || ($oldFold_name != $newFold_name && $count == 0)) {
			   		...
			   		if (Core::$resultOp->error == 0) {   	
			   			Sql::stripMagicFields($_POST);
			   			/* memorizza nel db */
			   			Sql::updateRawlyPost($App->params->fieldsFold,$App->params->tableFold,'id',$App->id);
			   			if (Core::$resultOp->error == 0) {		   			
			   				/* rinomina la cartella */		
			   				if ($oldFold_name != '') {
			   					if($oldFold_name != '' && file_exists ($App->itemUploadPathDir.$oldFold_name)) rename($App->itemUploadPathDir.$oldFold_name,$App->itemUploadPathDir.$newFold_name) or die('impossibile rinominare la cartella');
			   					} 	
		...
		if (Core::$resultOp->error > 0) {
			...
			$App->viewMethod = 'formMod';	
				...
				if (isset($_POST['submitForm'])) {	
					$App->viewMethod = 'list';
					Core::$resultOp->message = ucfirst($App->labels['fold']['item']).' modificat'.$App->labels['fold']['itemSex'].'!';								
					} else {						
						if (isset($_POST['id'])) {
							$App->id = $_POST['id'];
							$App->pageSubTitle = 'modifica '.$App->labels['fold']['item'];
...
switch((string)$App->viewMethod) {
	...
	case 'formNew':
		...
		$App->item->active = 1;
		if (Core::$resultOp->error > 0) Utilities::setItemDataObjWithPost($App->item,$fieldsFold);
	...
	case 'list':
		...
		$qryFields = array('c.*','(SELECT COUNT(i.id) FROM '.$App->tableItem.' AS i WHERE i.id_folder = c.id) AS numimages');
		$qryFieldsValues = array();
		$qryFieldsValuesClause = array();
		$clause = '';
		if (isset($_MY_SESSION_VARS[$App->sessionName]['srcTab']) && $_MY_SESSION_VARS[$App->sessionName]['srcTab'] != '') {
			list($sessClause,$qryFieldsValuesClause) = Sql::getClauseVarsFromAppSession($_MY_SESSION_VARS[$App->sessionName]['srcTab'],$fieldsFold,'');
			}		
		if (isset($sessClause) && $sessClause != '') $clause .= $sessClause;
		if (is_array($qryFieldsValuesClause) && count($qryFieldsValuesClause) > 0) {
			$qryFieldsValues = array_merge($qryFieldsValues,$qryFieldsValuesClause);	
			}
		Sql::initQuery($App->params->tableFold.' AS c',$qryFields,$qryFieldsValues,$clause);
