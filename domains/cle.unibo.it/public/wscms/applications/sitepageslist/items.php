...
switch(Core::$request->method) {
	...
	case 'moreOrderingItem':
		Utilities::increaseFieldOrdering($App->id,$_lang,array('table'=>$App->params->tableItem,'orderingType'=>$App->params->orderingItemType,'parent'=>1,'parentField'=>'id_cat','label'=>ucfirst($_lang['blocco']).' '.$_lang['spostato']));
		$_SESSION['message'] = '0|'.Core::$resultOp->message;
		ToolsStrings::redirect(URL_SITE_ADMIN.Core::$request->action.'/listItem');
	break;
	case 'lessOrderingItem':
		Utilities::decreaseFieldOrdering($App->id,$_lang,array('table'=>$App->params->tableItem,'orderingType'=>$App->params->orderingItemType,'parent'=>1,'parentField'=>'id_cat','label'=>ucfirst($_lang['blocco']).' '.$_lang['spostato']));
		$_SESSION['message'] = '0|'.Core::$resultOp->message;
		ToolsStrings::redirect(URL_SITE_ADMIN.Core::$request->action.'/listItem');
	break;
	...
	case 'activeItem':
	case 'disactiveItem':
		Sql::manageFieldActive(substr(Core::$request->method,0,-4),$App->params->tableItem,$App->id,array('label'=>$_lang['blocco'],'attivata'=>$_lang['attivato'],'disattivata'=>$_lang['disattivato']));
		$_SESSION['message'] = '0|'.Core::$resultOp->message;
		ToolsStrings::redirect(URL_SITE_ADMIN.Core::$request->action.'/listItem');
		die();
	break;
