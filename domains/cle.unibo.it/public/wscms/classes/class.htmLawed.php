...
function htmLawed($t, $C = 1, $S = [])
{
    ...
    $t = (($C['cdata'] or $C['comment']) && str_contains((string) $t, "\x01")) ? str_replace(["\x01", "\x02", "\x03", "\x04", "\x05"], ['', '', '&', '<', '>'], $t) : $t;
...
function hl_attrval($a, $t, $p)
{
    ...
    $r = [];
    $t = !empty($s) ? explode($s, (string) $t) : [$t];
    foreach ($t as $tk => $tv) {
        $o = 1;
        $tv = trim((string) $tv);
        $l = strlen($tv);
    ...
    $r = implode($s, $r);
    return (isset($r[0]) ? $r : ($p['default'] ?? 0));
}
        ...
        if (isset($x[0])) {
            if (strlen(trim($x)) && (($ql && isset($cB[$p])) or (isset($cB[$in]) && !$ql))) {
                ...
                foreach (preg_split('`(\x01\x02[^\x01\x02]+\x02\x01)`', $x, -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY) as $v) {
                    echo (str_starts_with($v, "\x01\x02") ? $v : ($do > 4 ? preg_replace('`\S`', '', $v) : ''));
                }
            } elseif ($do > 4) {
...
function hl_cmtcd($t)
{
    ...
    if ($n == 'comment' && $v < 4) {
        if (!str_ends_with((string) $t = preg_replace('`--+`', '-', substr((string) $t, 4, -3)), ' ')) {
            $t .= ' ';
        }
    } else {
...
function hl_ent($t)
{
    ...
    if (($n = ctype_digit($t = substr((string) $t, 1)) ? intval($t) : hexdec(substr($t, 1))) < 9 or ($n > 13 && $n < 32) or $n == 11 or $n == 12 or ($n > 126 && $n < 160 && $n != 133) or ($n > 55295 && ($n < 57344 or ($n > 64975 && $n < 64992) or $n == 65534 or $n == 65535 or $n > 1114111))) {
...
function hl_prot($p, $c = null)
{
    ...
    static $d = 'denied:';
    if (isset($c['!']) && substr((string) $p, 0, 7) != $d) {
        $p = "$d$p";
    }
    if (isset($c['*']) or !strcspn((string) $p, '#?;') or (substr((string) $p, 0, 7) == $d)) {
        return "{$b}{$p}{$a}";
    } // All ok, frag, query, param
    if (preg_match('`^([^:?[@!$()*,=/\'\]]+?)(:|&#(58|x3a);|%3a|\\\\0{0,4}3a).`i', (string) $p, $m) && !isset($c[strtolower($m[1])])) { // Denied prot
        return "{$b}{$d}{$p}{$a}";
    }
    if ($C['abs_url']) {
        if ($C['abs_url'] == -1 && str_starts_with((string) $p, (string) $C['base_url'])) { // Make url rel
            $p = substr((string) $p, strlen((string) $C['base_url']));
        } elseif (empty($m[1])) { // Make URL abs
            if (str_starts_with((string) $p, '//')) {
                $p = substr((string) $C['base_url'], 0, strpos((string) $C['base_url'], ':') + 1) . $p;
            } elseif ($p[0] == '/') {
                $p = preg_replace('`(^.+?://[^/]+)(.*)`', '$1', (string) $C['base_url']) . $p;
            } elseif (strcspn((string) $p, './')) {
                $p = $C['base_url'] . $p;
            } else {
                preg_match('`^([a-zA-Z\d\-+.]+://[^/]+)(.*)`', (string) $C['base_url'], $m);
                $p = preg_replace('`(?<=/)\./`', '', $m[2] . $p);
                while (preg_match('`(?<=/)([^/]{3,}|[^/.]+?|\.[^/.]|[^/.]\.)/\.\./`', (string) $p)) {
                    $p = preg_replace('`(?<=/)([^/]{3,}|[^/.]+?|\.[^/.]|[^/.]\.)/\.\./`', '', (string) $p);
                }
                $p = $m[1] . $p;
            }
        }
...
function hl_spec($t)
{
    ...
    $t = str_replace(["\t", "\r", "\n", ' '], '', preg_replace_callback('/"(?>(`.|[^"])*)"/sm', 'hl_aux1', trim((string) $t)));
...
function hl_tag($t)
{
    ...
    if (!preg_match('`^<(/?)([a-zA-Z][a-zA-Z1-6]*)([^>]*?)\s?>$`m', (string) $t, $m)) {
    ...
    // attr name-vals
    if (str_contains((string) $a, "\x01")) {
        $a = preg_replace('`\x01[^\x01]*\x01`', '', (string) $a);
    ...
    while (strlen((string) $a)) {
        ...
        switch ($mode) {
            case 0: // Name
                if (preg_match('`^[a-zA-Z][^\s=/]+`', (string) $a, $m)) {
                    $nm = strtolower($m[0]);
                    $w = $mode = 1;
    ...
    $d = $C['deny_attribute'];
    foreach ($aA as $k => $v) {
        if (((isset($d['*']) ? isset($d[$k]) : !isset($d[$k])) && (isset($aN[$k][$e]) or isset($aNU[$k]) or (isset($aNO[$k]) && !isset($d['on*'])) or (isset($aNA[$k]) && !isset($d['aria*'])) or (!isset($d['data*']) && preg_match('`data-((?!xml)[^:]+$)`', (string) $k))) && !isset($rl['n'][$k]) && !isset($rl['n']['*'])) or isset($rl[$k])) {
            if (isset($aNE[$k])) {
                $v = $k;
            ...
            if ($k == 'style' && !$C['style_pass']) {
                ...
                $v = preg_replace_callback('`(url(?:\()(?: )*(?:\'|"|&(?:quot|apos);)?)(.+?)((?:\'|"|&(?:quot|apos);)?(?: )*(?:\)))`iS', 'hl_prot', (string) $v);
                $v = !$C['css_expression'] ? preg_replace('`expression`i', ' ', (string) preg_replace('`\\\\\S|(/|(%2f))(\*|(%2a))`i', ' ', (string) $v)) : $v;
            } elseif (isset($aNP[$k]) or isset($aNO[$k])) {
                $v = str_replace("­", ' ', (str_contains((string) $v, '&') ? str_replace(['&#xad;', '&#173;', '&shy;'], ' ', $v) : $v)); # double-quoted char: soft-hyphen; appears here as "­" or hyphen or something else depending on viewing software
                if ($k == 'srcset') {
                    $v2 = '';
...
function hl_tag2(&$e, &$a, $t = 1)
{
    ...
    if ($e == 'font') {
        $a2 = '';
        while (preg_match('`(^|\s)(color|size)\s*=\s*(\'|")?(.+?)(\\3|\s|$)`i', (string) $a, $m)) {
            $a = str_replace($m[0], ' ', $a);
            $a2 .= strtolower($m[2]) == 'color' ? (' color: ' . str_replace('"', '\'', trim($m[4])) . ';') : (isset($fs[($m = trim($m[4]))]) ? ($a2 .= ' font-size: ' . str_replace('"', '\'', $fs[$m]) . ';') : '');
        }
        while (preg_match('`(^|\s)face\s*=\s*(\'|")?([^=]+?)\\2`i', (string) $a, $m) or preg_match('`(^|\s)face\s*=(\s*)(\S+)`i', (string) $a, $m)) {
            $a = str_replace($m[0], ' ', $a);
...
function hl_tidy($t, $w, $p)
{
    ...
    $t = preg_replace(['`(<\w[^>]*(?<!/)>)\s+`', '`\s+`', '`(<\w[^>]*(?<!/)>) `'], [' $1', ' ', '$1'], preg_replace_callback(['`(<(!\[CDATA\[))(.+?)(\]\]>)`sm', '`(<(!--))(.+?)(-->)`sm', '`(<(pre|script|textarea)[^>]*?>)(.+?)(</\2>)`sm'], 'hl_aux2', (string) $t));
    if (($w = strtolower((string) $w)) == -1) {
    ...
    while ($X) {
        ...
        for ($i = -1, $j = count($t); ++$i < $j;) {
            $r = '';
            [$e, $r] = explode('>', $t[$i]);
            $x = $e[0] == '/' ? 0 : (str_ends_with($e, '/') ? 1 : ($e[0] != '!' ? 2 : -1));
