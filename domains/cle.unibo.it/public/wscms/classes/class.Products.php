...
class Products extends Core {
	...
	public static function getProductDetails($id)
	{
		...
		if (Core::$resultOp->error > 0) { die('errore lettura dettagli prodotto');ToolsStrings::redirect(URL_SITE.'error/db'); die(); }
	...
	public static function getProductsList($id,$initClause = '') 
	{
		...
		while ($row = $pdoObject->fetch()) {
			...
			) 
			{
				...
				if (self::$getCompanyOwner == true) $row->companies_ragione_sociale = self::addCompanyOwnerFields($row->companies_code);
				$obj[] = $row;
			}			
		}
		//ToolsStrings::dump($obj);die();
