...
Sql::initQuery(DB_TABLE_PREFIX.'team_config',array('*'),array(),'');	
...
if (Core::$resultOp->error == 0) {
	switch (Core::$request->method) {
		...
		case 'dt':
			...
			Sql::initQuery(DB_TABLE_PREFIX.'team',array('*'),array($id),"active = 1 AND id = ?",'');
		...
		default:
			...
			while ($row = $pdoObject->fetch()) {
				$row->role = Multilanguage::getLocaleObjectValue($row,'role_',Config::$langVars['user'],array('htmLawed'=>1,'parse'=>1));
				$row->summary = Multilanguage::getLocaleObjectValue($row,'summary_',Config::$langVars['user'],array('htmLawed'=>1,'parse'=>1));
				$row->universita = Multilanguage::getLocaleObjectValue($row,'universita_',Config::$langVars['user'],array('htmLawed'=>1,'parse'=>1));
				$App->items[] = $row;		
			}
