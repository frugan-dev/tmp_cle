...
Sql::initQuery(Sql::getTablePrefix().'pages',array('*'),array('video'),'active = 1 AND (alias LIKE ?)');
$obj = Sql::getRecord();
...
$App->titles = Utilities::getTitlesPage(ucfirst($_lang['galleria']),$App->modulePageData,$_lang['user'],array());
//print_r($App->titles);
...
if (Core::$resultOp->error == 0) {
	switch (Core::$request->method) {								
		default:
			...
			$arr = array();
			if (Core::$resultOp->error == 0) {
				Sql::initQuery(DB_TABLE_PREFIX.'video',array('*'),array(),'active = 1','ordering ASC');
				$obj = Sql::getRecords();
				if (is_array($obj) && is_array($obj) && count($obj) > 0) {
					foreach ($obj AS $value) {		
						$value->title = Multilanguage::getLocaleObjectValue($value,'title_',$_lang['user'],array());
						$value->content = Multilanguage::getLocaleObjectValue($value,'content_',$_lang['user'],array());
						$value->tags = str_replace(',',' ',$value->id_tags);
					
						preg_match_all('/<iframe[^>]+src="([^"]+)"/', $value->embedded, $match);
